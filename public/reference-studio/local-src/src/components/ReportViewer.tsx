...
import type { AnalyticsResult, ChartConfig } from '../types';
    ...
    <div className="flex-1 p-8 space-y-16 overflow-auto bg-[#f8f8f8]" style={{ backgroundImage: 'radial-gradient(rgba(0,0,0,0.08) 1.5px, transparent 0)', backgroundSize: '40px 40px' }}>
       ...
       <div className="bg-white border-[6px] border-black rounded-[48px] shadow-[24px_24px_0px_0px_rgba(0,0,0,1)] p-12 space-y-20 relative overflow-hidden">
          ...
          <div className="absolute top-10 right-14 flex flex-col items-end gap-3 text-right">
             ...
             <p className="text-[10px] font-black uppercase tracking-[0.3em] leading-tight max-w-[200px] opacity-30">
               Advanced Algorithmic Fingerprinting & Strategy Module V21.4
             </p>
          </div>

          <div className="space-y-4">
             <div className="inline-flex items-center gap-2 bg-black text-[#CCFF00] px-4 py-1.5 rounded-full font-black uppercase text-xs tracking-widest">
                <Sparkles size={14} /> AI Diagnostic Report
             </div>
             <h1 className="text-7xl font-black uppercase tracking-tighter leading-none">The Strategy <span className="text-[#FF3399]">Pulse</span></h1>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
             <div className="lg:col-span-8 space-y-8">
                <div className="flex items-center gap-6">
                   <span className="bg-black text-white font-black px-4 py-2 rounded-2xl text-2xl tracking-tighter">01</span>
                   <h4 className="text-4xl font-black uppercase tracking-tighter">Executive Summary</h4>
                </div>
                <div className="prose prose-xl max-w-none">
                   <div className="mb-10 p-8 bg-black text-[#CCFF00] rounded-3xl border-[4px] border-black font-black text-lg leading-relaxed shadow-[12px_12px_0px_0px_rgba(204,255,0,0.2)]">
             ...
             <div className="lg:col-span-4 bg-white border-[6px] border-black rounded-[40px] p-10 shadow-[12px_12px_0px_0px_black] space-y-8">
                <div className="flex items-center gap-4">
                   <span className="bg-[#CCFF00] text-black font-black px-3 py-1 rounded-xl text-sm tracking-tighter">02</span>
                   <h5 className="font-black uppercase text-base tracking-widest">Channel Metrics</h5>
                </div>
                {result.stats && (
                  <div className="grid grid-cols-1 gap-6">
                     {Object.entries(result.stats).map(([key, val]) => (
                       <div key={key} className="flex items-center justify-between border-b-4 border-black/5 pb-4 group hover:border-[#FF3399] transition-colors">
                          ...
                          <span className="text-3xl font-black tracking-tighter tabular-nums group-hover:text-[#FF3399] transition-colors">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
             {result.sections.filter(s => !s.chartSuggestion).map((section, idx) => (
               <div key={idx} className="bg-white border-[5px] border-black rounded-[36px] overflow-hidden shadow-[16px_16px_0px_0px_black] flex flex-col hover:translate-y-[-8px] transition-transform cursor-default">
                  ...
                  <div className="p-10 font-bold text-lg leading-relaxed text-gray-700 flex-1">
          ...
          <div className="space-y-16 pt-10">
             <div className="flex items-center justify-between border-b-[6px] border-black pb-6">
                <h4 className="text-6xl font-black uppercase tracking-tighter">Deep Data <span className="text-[#00CCFF]">Sync</span></h4>
                   ...
                   <div key={idx} className="space-y-10 animate-in fade-in slide-in-from-bottom-8 duration-700" style={{ animationDelay: `${idx * 200}ms` }}>
                      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b-[5px] border-black pb-4">
                         <div className="space-y-2">
                            <h5 className="text-4xl font-black uppercase tracking-tight">{section.title}</h5>
                            <div className="flex items-center gap-3">
                         ...
                         {onLaunchVisualization && (
                           <button 
                              ...
                              className="bg-black text-[#CCFF00] font-black uppercase text-xs py-4 px-10 rounded-2xl shadow-[6px_6px_0px_0px_#FF3399] hover:shadow-none hover:translate-x-1 hover:translate-y-1 transition-all tracking-widest"
                           >
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
                         <div className="lg:col-span-5 font-bold text-lg text-gray-700 leading-relaxed bg-black/[0.03] p-10 rounded-[40px] border-[3px] border-dashed border-black/10">
                            <Markdown>{section.content}</Markdown>
                         </div>
             ...
             <div className="bg-white border-[6px] border-black rounded-[40px] overflow-hidden shadow-[20px_20px_0px_0px_black]">
                <div className="bg-[#CCFF00] p-10 border-b-[6px] border-black flex items-center justify-between">
                   <div className="space-y-1">
                      <h5 className="text-4xl font-black uppercase tracking-tighter">Efficiency Matrix</h5>
                     ...
                     <thead className="bg-black text-[#CCFF00] border-b-[4px] border-black">
                       <tr>
                         {result.keywordComparisonTable.headers.map((h, i) => (
                           <th key={i} className="p-6 border-r border-white/10 font-black uppercase tracking-tighter text-sm">{h}</th>
                         ))}
                       </tr>
                     </thead>
                     <tbody>
                       {result.keywordComparisonTable.rows.map((row, ri) => (
                         <tr key={ri} className="border-b-[4px] border-black/5 hover:bg-black/5 group transition-colors">
                           {row.map((cell, ci) => (
          ...
          {result.miniSpreadsheets && (
             <div className="space-y-12">
                <h4 className="text-4xl font-black uppercase tracking-tighter border-b-[5px] border-black pb-4">Comparative <span className="text-[#FF3399]">Micro-Data</span></h4>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                   {result.miniSpreadsheets.map((sheet, si) => (
                     <div key={si} className="bg-white border-[5px] border-black rounded-[32px] overflow-hidden shadow-[12px_12px_0px_0px_black] hover:rotate-1 transition-transform">
                       <div className={`px-6 py-4 border-b-[5px] border-black text-xs font-black uppercase tracking-widest ${
