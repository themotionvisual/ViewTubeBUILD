...
export const hasGeminiKey = (): boolean => {
  ...
  // @ts-ignore
  const envKey = import.meta.env?.VITE_GEMINI_API_KEY || "";
  return !!((customKey && customKey.trim() !== '') || envKey);
};
...
export const generateChatResponse = async (history: any[], newMessage: string, useThinking: boolean = false): Promise<string> => {
  const ai = getAiClient();
...
export const analyzeChannelData = async (
  ...
  for (const model of modelsToTry) {
    ...
    while (retries < maxRetries && !success) {
      try {
        ...
        const isTransient = errorMessage.includes('503') || errorMessage.includes('UNAVAILABLE') || errorMessage.includes('429');
        if (isTransient && retries < maxRetries) {
          retries++;
          const backoff = Math.pow(2, retries) * 1000;
          console.warn(`Attempt ${retries} failed for model ${model} with transient error. Retrying in ${backoff}ms...`, errorMessage);
          await new Promise(resolve => setTimeout(resolve, backoff));
        } else {
          console.warn(`Attempt failed with model ${model}:`, errorMessage);
          lastError = error;
  ...
  console.error("All models failed for Channel Analysis. Last error:", lastError);
  if (lastError?.message?.includes('INVALID_ARGUMENT')) {
