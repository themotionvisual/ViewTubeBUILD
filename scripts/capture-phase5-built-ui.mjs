  ...
  for (const target of targets) {
    ...
    for (const path of target.paths) {
      ...
      try {
        ...
        const bad = !response || response.status() >= 400 || /404|not found|page does not exist/i.test(text);
        if (bad) continue;
        const file = `${out}/${target.name}-${vp.label}.png`;
        await page.screenshot({ path: file, fullPage: true });
        manifest.push({ target: target.name, viewport: vp.label, path, url, file, status: response.status() });
        captured = true;
        break;
      } catch {}
    }
    if (!captured) manifest.push({ target: target.name, viewport: vp.label, error: 'No candidate route rendered successfully.' });
  }
  await context.close();
}
await browser.close();
