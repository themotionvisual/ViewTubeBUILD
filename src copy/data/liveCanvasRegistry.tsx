...
const liveCanvasRegistry: Record<string, LiveCanvasSpec> = {
  'components/Toolbox.tsx (AccordionContainer)': {
  ...
  'components/ToolboxUISystem.tsx': { label: 'ToolboxUISystem (Section 1)', render: () => <ToolboxUISystem /> },
  'src/components/ToolboxUISystem.tsx': { label: 'ToolboxUISystem (Section 1)', render: () => <ToolboxUISystem /> },
  'components/UIReferenceLibraryContent.tsx': { label: 'Section 1 Component/UI Live Canvas', render: () => <ToolboxUISystem /> },
  ...
  'views/Settings.tsx': { label: 'Settings', render: () => <Settings /> },
};

export const getLiveCanvasSpec = (path: string): LiveCanvasSpec | null => {
  const normalized = path.replace(/^\.\//, '').replace(/^src\//, '');
  return (
    liveCanvasRegistry[path] ||
    liveCanvasRegistry[normalized] ||
    liveCanvasRegistry[`src/${normalized}`] ||
    null
  );
};

export const getLiveCanvasOptions = (): Array<{ key: string; label: string }> => {
  return Object.entries(liveCanvasRegistry).map(([key, spec]) => ({
    key,
    label: spec.label,
  }));
};
