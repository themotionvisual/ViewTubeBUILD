...
**Last audited main commit:** 052ff0294b5f8c9b05867a85ab4e2d407a27ec06  
...
| Mobile/desktop capability parity | started | editor state + UI | round-trip fixtures + capability matrix | FX capability entries now record per-surface truth; expand evidence-backed matrix across all capabilities |
| Shared FX/transition catalogs | partial | editor core | same IDs and rendering on both surfaces | transition + FX canonical contracts implemented; wire/verify desktop FX surface before calling parity complete |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 2026-09-24 | ChatGPT Phase A shared-FX pass | feat/editor-phase-a-shared-fx-2026-09-24 | started | Continued after PR #405 merged (`052ff029...`). Added canonical FX catalog for blur/saturation/brightness/hue/contrast/sepia/grayscale/opacity; refactored mobile FX rack, mobile preview, Remotion final composition, mobile keyframe evaluation, and render-worker keyframe vocabulary to the shared contract; added per-surface capability truth so active mobile/preview/render FX do not masquerade as desktop parity. | src/shared/vtE1FxCatalog.*; EffectsLibrariesPanel.tsx; MobileProjectPreview.tsx; mobilePreviewGeometry.ts; Composition.tsx; vt-e1-render-server.mjs; editorCapabilities.ts + test | source audit against main 052ff029...; targeted tests added; CI pending | canonical FX contract + Phase A parity audit | open PR; verify no branch-introduced CI failures; then add rich whole-project round-trip fixture |
