...
| Preview ↔ final Remotion parity | partial | editor + remotion | frame fixtures pass | transition, FX, transform/crop/order are merged; current audio slice unifies volume/fade/mute/rate; next certify templates/assets and rendered fixtures |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 2026-09-24 | ChatGPT Phase A audio parity pass | feat/editor-phase-a-audio-parity-v2-2026-09-24 | started | Added shared audio-frame contract; made standalone audio audible in mobile preview; unified volume keyframes, fades, mute and playback-rate semantics across preview/final; fixed final track-mute behavior so visuals remain renderable; expanded mobile audio controls and rich fixture. | src/shared/vtE1AudioFrame.*; MobileProjectPreview.tsx; ClipSettingsPanel.tsx; editorState.ts; Composition.tsx; editorProjectRichParity.test.ts; Phase A audit | original audio branch was replayed without overlap onto current main bc948ec3... as v2; branch is ahead-only / behind 0; CI pending | PR #417 visual-frame contract + Phase A audit | open bounded PR, run gates, fix only branch-introduced failures, merge deliberately, then template/Remotion-asset rendered parity |
| 2026-09-24 | ChatGPT Phase A visual-frame parity pass | PR #417 / feat/editor-phase-a-visual-frame-parity-v2-2026-09-24 | completed | Added canonical shared visual-frame contract for keyframe easing/evaluation, clip transform, crop and track ordering. Refactored mobile preview and final Remotion output to consume it; preserved track order through the desktop/mobile bridge; added focused deterministic fixtures. | src/shared/vtE1VisualFrame.*; mobilePreviewGeometry.ts; MobileProjectPreview.tsx; editorState.ts; editorDesktopProjectAdapter.ts + test; Composition.tsx; Phase A audit | merged to main as 4495d84c...; production-build/source-governance/local-smoke passed; remaining release-gate failures were existing unrelated baseline debt; Vercel preview quota remained blocked | Phase A parity audit + shared transition/FX contracts + PR #417 | continue audio parity, then template/assets rendered parity and Phase B UI consolidation |
