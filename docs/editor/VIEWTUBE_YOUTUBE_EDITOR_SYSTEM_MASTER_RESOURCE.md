...
| Shared FX/transition catalogs | partial | editor core | same IDs and rendering on both surfaces | transition catalog/frame contract implemented; build shared FX catalog next |
| Preview ↔ final Remotion parity | started | editor + remotion | frame fixtures pass | canonical transition frame fixtures added; expand to transforms/crop/keyframes/assets/audio |
