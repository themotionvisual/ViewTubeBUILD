...
| Mobile/desktop capability parity | started | editor state + UI | rich round-trip fixture + capability matrix | rich fixture now covers tracks, layers, FX, keyframes, templates, Remotion assets, audio, captions, transitions, ContentBuild identity and forward-compatible metadata; expand matrix across all capabilities |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 2026-09-24 | ChatGPT Phase A rich-parity pass | feat/editor-phase-a-rich-parity-fixtures-2026-09-24 | started | Began next Phase A slice after PR #408 merged (`56e2c0c...`). Added a rich desktop→mobile→desktop project fixture covering video/audio/overlay/caption tracks, layers, crop/transform/FX, keyframes, design-template clips, Remotion asset clips, audio metadata, captions, transitions, ContentBuild/legacy IDs, asset selections, generation provenance, and unknown desktop fields. Also repaired bounded editor static-quality debt in the render client, export project ID narrowing, motion-keyframe value narrowing, bridge test layer normalization, and desktop timeline adapter declarations. | editorProjectRichParity.test.ts; renderJobContract.ts; ExportRenderPanel.tsx; PreviewPane.tsx; useDesktopProjectBridge.test.tsx; desktopTimelineAdapter.d.ts | branch based on main `56e2c0c...`; CI pending | Phase A parity audit + canonical transition/FX contracts | open PR and verify targeted tests/type errors; then expand preview/final parity beyond transitions+FX |
