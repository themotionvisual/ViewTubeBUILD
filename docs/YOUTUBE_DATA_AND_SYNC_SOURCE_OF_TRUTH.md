...
| **Subscribers** | Subscribers Gained | `subscribersGained` | `subscribers_gained` / `Subscribers gained` | count | All | ✅ | Gross gained subscribers for the row scope. Distinct from net subscribers. |
| **Subscribers** | Subscribers Lost | `subscribersLost` | `subscribers_lost` / `Subscribers lost` | count | All | ✅ | Gross lost subscribers for the row scope. Distinct from net subscribers. |
| **Subscribers** | Subscribers Net | `subscribersNet` | `Subscribers` | count | All | ❌ | Net subscriber delta for the row scope when present in Studio CSV exports. Do not alias to gross gained subscribers. |
...
### 4.1.1 Historical CSV vs Partial API Cache Arbitration
When `/performance` is in merged mode, ViewTube must not assume the API cache and uploaded CSV package cover the same historical row set.

Rules:
* **Primary row identity** is exact `videoId`.
* **Title fallback matching** is allowed only when both sides lack a `videoId`.
* Rows present in CSV but absent from the current API snapshot are treated as `csv_only` or `hybrid_partial`, not full hybrid rows.
* Long-only metrics such as `Impressions` and `Impressions click-through rate (%)` must not be rendered as applicable Shorts fields.
* Shorts-only metrics such as `Stayed to watch (%)` must not be reused as AVP or generic retention aliases.
