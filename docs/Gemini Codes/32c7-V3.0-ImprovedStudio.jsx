...
const useAIIntegration = () => {
  ...
  const callGemini = async (promptText, currentAsset = null, retryCount = 0) => {
    const systemPrompt = `You are a professional Motion Graphics Designer and React Developer. 
    Analyze the user's prompt to determine if they want to create a NEW asset or MODIFY the existing one.
    
    Current Asset State: ${currentAsset ? JSON.stringify(currentAsset) : 'None'}
    
    Instructions:
    1. If the prompt asks for modification (e.g., "make circles blue", "change to rocket"), UPDATE current state
    2. If the prompt asks for new object (e.g., "a bicycle", "a flag"), CREATE new shapes
    ...
    JSON Schema:
    {
      ...
      "styleVariant": "neo-brutalist" | "pop" | "minimal",
      "shapes": Array<{
    ...
    try {
      ...
      if (retryCount < 3) {
        ...
        return callGemini(promptText, currentAsset, retryCount + 1);
      }
      throw err;
    }
  };
...
export default function UnifiedMotionStudio() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const result = await callGemini(createPrompt, null);
      setAsset(result);
      setIsPlaying(false);
      setTimeout(() => setIsPlaying(true), 50);
      setCreatePrompt("");
  ...
  const handleModify = async () => {
    ...
    try {
      const result = await callGemini(modifyPrompt, asset);
      setAsset(prev => ({ ...prev, ...result }));
      setIsPlaying(false);
      setTimeout(() => setIsPlaying(true), 50);
      setModifyPrompt("");
...
export default GeneratedAsset;
    `;
    console.log('%c=== EXPORTED CODE ===', 'color: #C9F830; font-weight: bold; font-size: 14px;');
    console.log(code);
    console.log('%c=====================', 'color: #C9F830; font-weight: bold; font-size: 14px;');
    console.log('%cCode exported to console. Copy from there!', 'color: #24D3FF; font-weight: bold;');
  };

  const exportVideo = () => {
    console.log('%c=== EXPORT VIDEO ===', 'color: #FF7497; font-weight: bold; font-size: 14px;');
    console.log(`%cExporting ${exportFormat} at ${exportQuality} quality...`, 'color: #C9F830; font-weight: bold;');
    console.log('%c====================', 'color: #FF7497; font-weight: bold; font-size: 14px;');
  };
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen bg-[#0a0a0a] text-white font-sans overflow-hidden select-none">
      ...
      `}>
        ...
        {/* AI Generation Section - Create */}
        <div className="bg-gradient-to-br from-[#1e1e1e] to-[#2a2a2a] border-2 border-black rounded-xl p-4 shadow-[4px_4px_0px_0px_black] mb-4">
          <h3 className="text-[10px] font-black uppercase text-[#C9F830] mb-3 flex items-center gap-2">
            <Zap size={14} /> AI GENERATE (New Asset)
          </h3>
          <textarea 
            value={createPrompt}
            onChange={(e) => setCreatePrompt(e.target.value)}
            placeholder="e.g. 'Create a rocket ship with neon trails...'"
            ...
            rows={2}
          />
          <button onClick={handleGenerate} className="w-full h-10 bg-[#C9F830] text-black border-2 border-black rounded-lg font-black uppercase text-xs active:translate-y-0.5 transition-all hover:shadow-[2px_2px_0px_0px_black] mt-2">
            {isGenerating ? <RefreshCcw className="animate-spin mx-auto" size={16} /> : <><Zap size={16} className="inline mr-1" /> Generate</>}
          </button>
        </div>

        {/* AI Modification Section - Modify */}
        <div className="bg-gradient-to-br from-[#1e1e1e] to-[#2a2a2a] border-2 border-black rounded-xl p-4 shadow-[4px_4px_0px_0px_black] mb-4">
          <h3 className="text-[10px] font-black uppercase text-[#24D3FF] mb-3 flex items-center gap-2">
            <Wand2 size={14} /> AI MODIFY (Current Asset)
          </h3>
          <textarea 
            value={modifyPrompt}
            onChange={(e) => setModifyPrompt(e.target.value)}
            placeholder="e.g. 'Make the circles blue and add flames...'"
            ...
            rows={2}
          />
          <button onClick={handleModify} className="w-full h-10 bg-[#24D3FF] text-black border-2 border-black rounded-lg font-black uppercase text-xs active:translate-y-0.5 transition-all hover:shadow-[2px_2px_0px_0px_black] mt-2">
            {isGenerating ? <RefreshCcw className="animate-spin mx-auto" size={16} /> : <><Wand2 size={16} className="inline mr-1" /> Modify</>}
          </button>
        </div>
        ...
        <div className="bg-[#1e1e1e] border-2 border-black rounded-xl p-4 mb-4 space-y-4">
          ...
          <div className="space-y-3">
            <div className="space-y-1">
              <label className="text-[8px] font-black uppercase text-gray-400">Primary Text (Title)</label>
              <input 
                type="text" 
                value={asset.primary}
                onChange={(e) => setAsset({...asset, primary: e.target.value})}
                className="w-full bg-black/50 border-2 border-black rounded-lg p-2 text-sm font-bold focus:border-[#C9F830] outline-none"
              />
            </div>
            <div className="space-y-1">
              <label className="text-[8px] font-black uppercase text-gray-400">Secondary Text (Handle)</label>
              <input 
                type="text" 
        ...
        <div className="bg-[#1e1e1e] border-2 border-black rounded-xl p-4 mb-4 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-2 mb-3">
            {ANIMATIONS.map(a => (
              <button 
                key={a.id}
                onClick={() => { setAsset({...asset, animationIn: a.id}); triggerReplay(); }}
      ...
      <main className={`flex-1 relative flex flex-col bg-[#050505] p-4 md:p-6 overflow-hidden items-center justify-center ${isFullscreen ? 'fixed inset-0 z-50' : ''}`}>
        ...
        <header className="h-12 bg-[#141414] border-b-2 border-black flex items-center justify-between px-4 md:px-6 shrink-0 w-full">
          <div className="flex items-center gap-2">
            ...
            <button onClick={resetAsset} className="ml-2 px-3 py-1 bg-gray-600 border-2 border-black rounded-lg text-[10px] font-black uppercase hover:bg-gray-500 transition-all">
              Reset
            </button>
          </div>
        ...
        {isGenerating && (
          <div className="absolute inset-0 bg-black/80 backdrop-blur-md z-[100] flex flex-col items-center justify-center gap-6">
             ...
             <div className="text-center">
               <p className="font-black uppercase tracking-[0.2em] text-lg md:text-xl text-[#C9F830]">AI Generating Asset</p>
               <p className="text-gray-500 font-bold text-xs md:text-sm mt-2 uppercase">Building Geometry with Smart Analysis</p>
             </div>
          </div>
        )}
