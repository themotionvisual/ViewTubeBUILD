...
4. **Analysis-Only Media Compression Rule:** For uploads used only for AI analysis (not publishing), route file through `/api/media/compress-analysis` first and send only compressed derivative to AI. Never send raw upload as silent fallback.
