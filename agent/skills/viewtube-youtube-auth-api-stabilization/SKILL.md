---
name: viewtube-youtube-auth-api-stabilization
description: Change, debug, merge, remove or review ViewTube code involving Google OAuth, YouTube authorization, the YouTube Data/Analytics/Reporting APIs, account and channel connection state, API proxies, token and session handling, or the deployment routes that serve them. Use whenever work touches sign-in, connect/disconnect, scopes, refresh tokens, quota, auth diagnostics, or a route returning 401/403/404 from a YouTube or account endpoint.
---
