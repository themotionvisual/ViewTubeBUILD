...
export const executeAlgorithmRecommendationForBrain = async (input: {
 ...
 if (!controls.enabled) {
  return { status: "disabled" as const, message: "ViewTube Brain is disabled by the creator." }
 }
 const recommendation = input.portfolio.recommendations.find((candidate) => candidate.id === input.recommendationId)
 if (!recommendation) {
  return { status: "not_found" as const, message: "Algorithm recommendation was not found." }
 }
...
export const executePrimingStepForBrain = async (input: {
 ...
 if (!canBrainUseCapability(controls, "projects")) {
  return { status: "projects_disabled" as const, message: "Project access is disabled in Brain User Controls." }
 }
 if (!input.portfolio.primingPlan) {
  return { status: "not_found" as const, message: "No active priming plan exists in this portfolio." }
 }
 return createPrimingStepHandoff({
  plan: input.portfolio.primingPlan,
  stepId: input.stepId,
  creatorApproved: input.creatorApproved,
  creatorDecisions: input.creatorDecisions,
 })
}
