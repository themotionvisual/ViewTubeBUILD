...
import {
 listAlgorithmLifecycleObservations,
 type AlgorithmLifecycleObservation,
} from "./AlgorithmLifecycleObservationStore"
import {
 listDueAlgorithmMonitoringCheckpoints,
 type AlgorithmMonitoringCheckpoint,
} from "./AlgorithmMonitoringSchedule"
...
const observationToleranceMs = (checkpoint: AlgorithmMonitoringCheckpoint) => {
 // Early checkpoints need tighter evidence; later checkpoints can tolerate a wider
 // sync cadence without pretending a materially later lifetime total is T+1h/T+6h.
...
export const resolveAlgorithmMonitoringCheckpoint = (input: {
 ...
 if (!event.videoId) {
  return {
   status: "not_video_scoped",
   sourceEventId: event.id,
   checkpointId: checkpoint.id,
   checkpoint,
   metrics: [],
   missingMetrics: checkpoint.metrics,
   event: null,
  }
 }

 const observations = input.observations || listAlgorithmLifecycleObservations({
  channelId: event.channelId,
  videoId: event.videoId,
 })
 const matches = checkpoint.metrics
  .map((metric) => ({ metric, row: closestObservation({ observations, event, checkpoint, metric }) }))
 const resolvedMetrics: ResolvedMonitoringMetric[] = matches
  ...
  .map(({ metric, row }) => ({
   ...
   lifecycleHour: row.lifecycleHour,
 ...
 if (missingMetrics.length) {
  return {
   status: resolvedMetrics.length ? "partial" : "missing_evidence",
   sourceEventId: event.id,
   checkpointId: checkpoint.id,
   checkpoint,
 ...
 const recorded = recordAlgorithmIntelligenceEvent({
  ...
  evidenceIds: [
   ...event.evidenceIds,
   ...resolvedMetrics.map((metric) => metric.evidenceId).filter(Boolean) as string[],
  ],
...
/**
 * Resolves due monitoring checkpoints only when the required video-scoped
 * lifecycle evidence has actually been captured. Partial evidence remains visible
 * as unresolved; no values are synthesized and no OUTCOME_MEASURED event is made.
 */
export const resolveDueAlgorithmMonitoringCheckpoints = (input: {
 channelId: string
 now?: number
 maximum?: number
}) => {
 const due = listDueAlgorithmMonitoringCheckpoints({
  channelId: input.channelId,
  now: input.now,
  maximum: input.maximum,
 })
 const observations = listAlgorithmLifecycleObservations({ channelId: input.channelId })
 const resolutions = due.map(({ event, checkpoint }) => resolveAlgorithmMonitoringCheckpoint({
  event,
  checkpoint,
  observations,
 }))
