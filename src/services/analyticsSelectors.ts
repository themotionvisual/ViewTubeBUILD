...
export function getMasterRows(
  ...
  if (sourceMode === "api") {
    return apiRowsForWindow(cache, window)
  }
  if (sourceMode === "csv") {
    return csvRowsForWindow(csvFiles, window)
  }
...
const apiRowsForWindow = (
  ...
  metadataByVideoId.forEach((meta, videoId) => {
    ...
    let row: CanonicalVideoRow = {
      ...
      title,
      uploadDate,
      format: resolveCanonicalVideoFormat(contentType, durationSeconds, isShortSignal, metadataShortSignal, ""),
      durationSeconds,
      sourceMode: "api",
      metrics: metricCells,
    }
  ...
  reportRows.forEach((rawRow, index) => {
    ...
    let row: CanonicalVideoRow = {
      ...
      format: resolveCanonicalVideoFormat(text(rawRow.creatorContentType), durationSeconds, false, hasMetadataShortSignal(text(rawRow["Video title"] ?? rawRow.title), text(rawRow.description), Array.isArray(rawRow.tags) ? rawRow.tags.map((tag) => text(tag)).filter(Boolean) : []), ""),
      durationSeconds,
      sourceMode: "api",
  ...
  filesToUse.forEach(({ file, rows }) => {
    rows.forEach((rawRow, index) => {
      ...
      let row: CanonicalVideoRow = {
        ...
        format: resolveCanonicalVideoFormat(text(normalized.Format ?? normalized.Type ?? normalized.contentType ?? normalized.creatorContentType), durationSeconds, false, false, text(file.tag)),
        durationSeconds,
...
export const MASTER_TABLE_HEADER_TO_METRIC_KEY: Record<string, CanonicalMetricKey> = canonicalMetricOrder.reduce((acc, metricKey) => {
...
export const canonicalRowsToMasterTableRows = (rows: CanonicalVideoRow[]): MasterTableRow[] => {
  return rows.map((row, index) => {
    const base: MasterTableRow = {
      ...
      _sourceFile: row.sourceMode === "api" ? "YouTube API" : row.sourceMode === "csv_table" ? "CSV Upload" : "YouTube API + CSV",
      _userTag: row.format,
