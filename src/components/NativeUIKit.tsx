  ...
  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
   {[
    ...
    },
    {
     title: "Hero Box",
     color: "bg-[#00CCFF]",
     shadow: "shadow-[12px_12px_0px_0px_black]",
    },
    {
     title: "Compact Box",
     color: "bg-[#FFDD00]",
    ...
    <div
     key={b.title}
     className={`border-[4px] border-black rounded-2xl ${b.shadow} overflow-hidden bg-white`}>
     <div
      className={`${b.color} h-14 border-b-[4px] border-black flex items-center px-4`}>
      <span className="font-[1000] uppercase tracking-tighter text-lg">
       {b.title}
      </span>
     </div>
     <div className="p-6">
 ...
 return (
  <div className="space-y-6">
   ...
   <div className="flex flex-wrap gap-4">
    ...
    <button
     disabled
     className="bg-gray-200 text-black/20 px-6 py-3 border-[4px] border-black/20 rounded-2xl font-[1000] uppercase tracking-tighter text-sm cursor-not-allowed">
     Disabled
    </button>
   </div>
   {clicked && (
    ...
    {["Publish Alerts", "Auto Thumbnails", "Dark Analytics"].map((label, i) => (
     <div key={label} className="flex items-center gap-4">
      <div
       ...
       <div
        className={`w-6 h-6 bg-white border-[3px] border-black rounded-full absolute top-[1px] transition-all ${toggles[i] ? "left-[22px]" : "left-[1px]"}`}
       />
      </div>
   ...
   <div className="space-y-3">
    ...
    {checkLabels.map((label, i) => (
     <div
      ...
      <div
       className={`w-7 h-7 border-[3px] border-black rounded-lg flex items-center justify-center transition-all ${checks[i] ? "bg-[#00CCFF]" : "bg-white group-hover:bg-gray-50"}`}>
       {checks[i] && <span className="text-black font-black text-sm">✓</span>}
      </div>
      <span
       className={`font-black uppercase text-xs tracking-tighter transition-all ${checks[i] ? "text-black line-through" : "text-black/60"}`}>
       {label}
      </span>
     </div>
    ))}
   ...
   <div className="space-y-3">
    ...
    {["Growth Mode", "Monetize Mode", "Engagement Mode"].map((label, i) => (
     <div
      ...
      <div
       className={`w-7 h-7 border-[3px] border-black rounded-full flex items-center justify-center ${radio === i ? "bg-[#FF3399]" : "bg-white"}`}>
 ...
 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
  <div className="space-y-2">
   ...
   <input
    ...
    className="w-full border-[4px] border-black p-3 font-black uppercase text-lg rounded-xl outline-none focus:bg-[#CCFF00]/10 transition-colors"
   />
  </div>
  <div className="space-y-2">
   <label className="text-[10px] font-black uppercase tracking-widest text-black/30">
   ...
   <input
    placeholder="UC..."
    className="w-full border-[4px] border-black p-3 font-black uppercase text-lg rounded-xl outline-none focus:bg-[#00CCFF]/10 transition-colors"
   />
  </div>
  <div className="space-y-2 md:col-span-2">
   ...
   <textarea
    ...
    className="w-full border-[4px] border-black p-3 font-bold text-sm rounded-xl outline-none focus:bg-[#FF3399]/5 resize-none transition-colors"
   />
  </div>
  <div className="space-y-2">
   <label className="text-[10px] font-black uppercase tracking-widest text-black/30">
   ...
   {labels.map((label, i) => (
    <div key={label} className="space-y-2">
     <div className="flex justify-between">
      ...
      <span
       className="text-sm font-[1000] uppercase"
     ...
     <div key={b.label + "-ring"} className="flex flex-col items-center gap-2">
      <svg width="80" height="80" viewBox="0 0 100 100">
       ...
       <circle
        ...
        stroke={b.color}
        strokeWidth="10"
        strokeDasharray={`${b.pct * 2.39} ${239 - b.pct * 2.39}`}
 ...
 return (
  <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
   {stats.map((s) => (
    <div
     key={s.label}
     className={`${s.color} border-[4px] border-black rounded-2xl p-4 shadow-[4px_4px_0px_0px_black] hover:shadow-none hover:translate-y-1 transition-all cursor-default`}>
     <p className="text-[9px] font-black uppercase tracking-widest text-black/40 mb-1">
      {s.label}
     </p>
     <p className="text-3xl font-[1000] uppercase tracking-tighter leading-none">
      {s.value}
     </p>
     <p
      className={`text-[10px] font-black uppercase mt-1 ${s.up ? "text-black/60" : "text-red-600/60"}`}>
      {s.delta}
     </p>
    </div>
   ))}
 ...
 const rows = [
  {
   ...
   ctr: "8.1%",
   date: "May 18",
   children: ["Episode 1: Beach Day", "Episode 2: Sunset Hike"],
  },
  {
   ...
   <table className="w-full text-left">
    <thead>
     <tr className="bg-white border-b-[4px] border-black">
      ...
      <th className="p-3 font-black uppercase text-[10px] tracking-widest text-black/40">
       Title
      </th>
      <th className="p-3 font-black uppercase text-[10px] tracking-widest text-black/40">
       Status
       ...
       <tr className="border-b-[2px] border-black/10 hover:bg-[#CCFF00]/10 transition-colors">
        <td className="p-3">
         {r.children.length > 0 && (
          <button
           onClick={() => toggle(i)}
           className={`font-black text-xs transition-transform ${expanded.includes(i) ? "rotate-90" : ""}`}>
           ▶
          </button>
         )}
        </td>
        <td className="p-3 font-black uppercase text-sm tracking-tighter">
         {r.title}
        </td>
        <td className="p-3">
         <span
          className={`${r.statusColor} px-3 py-1 border-[2px] border-black rounded-lg text-[9px] font-black uppercase`}>
          {r.status}
         </span>
        </td>
       ...
       {expanded.includes(i) &&
        r.children.map((child, ci) => (
         <tr key={ci} className="bg-gray-50 border-b border-black/5">
          <td className="p-3"></td>
          <td className="p-3 pl-8 text-xs font-bold text-black/50" colSpan={5}>
           ↳ {child}
          </td>
         </tr>
        ))}
  ...
  <div className="max-w-sm">
   <div className="flex items-center justify-between mb-4">
    <button
     ...
     className="w-10 h-10 border-[3px] border-black rounded-xl font-black flex items-center justify-center hover:bg-[#CCFF00] transition-colors">
     ◀
    </button>
   ...
   <div className="grid grid-cols-7 gap-1 mb-1">
    {["S", "M", "T", "W", "T", "F", "S"].map((d, i) => (
     <div
      key={i}
      className="text-center text-[9px] font-black uppercase tracking-widest text-black/30 py-1">
      {d}
     </div>
    ))}
   </div>
     ...
     return (
      <button
       ...
       className={`aspect-square rounded-xl text-sm font-black flex items-center justify-center transition-all border-[2px] ${
        isSel
         ? "bg-[#FF3399] text-white border-black scale-110"
         : isToday
           ? "bg-[#CCFF00] border-black"
           : isEvent
             ? "bg-[#00CCFF]/20 border-[#00CCFF] hover:bg-[#00CCFF]/40"
   ...
   <div className="flex gap-4 mt-4">
    <div className="flex items-center gap-2">
     <div className="w-3 h-3 bg-[#FF3399] rounded-full border-[2px] border-black" />
     <span className="text-[9px] font-black uppercase tracking-widest text-black/40">
      Post Day
     </span>
    </div>
    <div className="flex items-center gap-2">
     <div className="w-3 h-3 bg-[#CCFF00] rounded-full border-[2px] border-black" />
     <span className="text-[9px] font-black uppercase tracking-widest text-black/40">
      Today
     </span>
    </div>
     ...
     <button
      ...
      className={`flex-1 py-3 font-[1000] uppercase tracking-tighter text-sm transition-all ${
       tab === i
        ...
        : "bg-white text-black/30 hover:bg-gray-50 hover:text-black"
      } ${i > 0 ? "border-l-[3px] border-black" : ""}`}>
      {t}
     </button>
    ))}
   </div>
   <p className="text-xs font-bold uppercase text-black/30 tracking-widest">
    Tab {tab + 1} content renders here
   </p>
    ...
    {["All", "Long", "Shorts"].map((p, i) => (
     <button
      ...
      className={`px-5 py-2 rounded-full border-[3px] border-black font-black uppercase text-[10px] tracking-tighter transition-all ${
       pill === i
        ? "bg-[#FF3399] text-white shadow-none"
        : "bg-white text-black shadow-[3px_3px_0px_0px_black] hover:shadow-none hover:translate-y-0.5"
      }`}>
      {p}
     </button>
    ))}
 ...
 <div className="space-y-6">
  <div className="flex flex-wrap gap-2">
   {[
    ...
    <span
     key={b.label}
     className={`${b.bg} ${b.text} px-3 py-1 border-[2px] border-black rounded-lg text-[10px] font-black uppercase tracking-wider`}>
     {b.label}
    </span>
   ))}
  </div>
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
    ...
    <div className="flex items-center gap-1">
     {[1, 2, 3, 4, 5].map((i) => (
      <button
       key={i}
       onClick={() => setRating(i)}
       className={`text-3xl transition-all hover:scale-125 ${i <= rating ? "text-[#FFDD00]" : "text-black/10"}`}>
       ★
      </button>
     ))}
   ...
   <div className="space-y-3">
    ...
    <div className="bg-[#CCFF00] border-[3px] border-black rounded-xl px-4 py-3 flex items-center gap-3 shadow-[4px_4px_0px_0px_black]">
     ...
     <span className="font-black cursor-pointer hover:text-red-500">✕</span>
    </div>
    <div className="bg-[#FF3399] text-white border-[3px] border-black rounded-xl px-4 py-3 flex items-center gap-3 shadow-[4px_4px_0px_0px_black]">
     <span className="font-black">✕</span>
     <span className="font-bold text-sm flex-1">Upload failed. Try again.</span>
     <span className="font-black cursor-pointer">✕</span>
    </div>
    <div className="bg-[#00CCFF] border-[3px] border-black rounded-xl px-4 py-3 flex items-center gap-3 shadow-[4px_4px_0px_0px_black]">
     <span className="font-black">ℹ</span>
     <span className="font-bold text-sm flex-1">AI tactics generated.</span>
     <span className="font-black cursor-pointer hover:text-red-500">✕</span>
    </div>
   </div>
  </div>
 ...
 <div className="flex flex-wrap gap-6 items-end">
  {[
   ...
   <div key={a.label} className="flex flex-col items-center gap-2">
    <div
     className={`w-16 h-16 ${a.color} border-[4px] border-black rounded-2xl flex items-center justify-center text-2xl shadow-[4px_4px_0px_0px_black] ${a.anim}`}>
     {a.emoji}
    </div>
    <span className="text-[9px] font-black uppercase tracking-widest text-black/40">
     {a.label}
 ...
 return (
  <>
   <button
    ...
    className="bg-[#FF3399] text-white px-8 py-4 border-[4px] border-black rounded-2xl font-[1000] uppercase tracking-tighter shadow-[8px_8px_0px_0px_black] hover:shadow-none hover:translate-x-2 hover:translate-y-2 transition-all text-lg">
    Open Modal ↗
   </button>
   {open && (
    <div
     ...
     <div className="bg-white border-[5px] border-black rounded-2xl shadow-[16px_16px_0px_0px_black] w-full max-w-md overflow-hidden">
      <div className="bg-[#FFDD00] px-6 py-4 border-b-[4px] border-black flex justify-between items-center">
       ...
       <button
        ...
        className="w-10 h-10 bg-white border-[3px] border-black rounded-xl font-black flex items-center justify-center hover:bg-[#FF3399] hover:text-white transition-all">
        ✕
       </button>
      </div>
      <div className="p-6 space-y-4">
       <p className="font-bold text-sm leading-relaxed">
        AI will scan your channel and generate personalized success actions
        based on your goals.
       </p>
      ...
      <div className="px-6 py-4 border-t-[3px] border-black/10 flex justify-end gap-3">
       <button
        onClick={() => setOpen(false)}
        className="px-6 py-2 border-[3px] border-black rounded-xl font-black uppercase text-xs hover:bg-gray-50 transition-colors">
        Cancel
       </button>
       <button className="px-6 py-2 bg-[#FF3399] text-white border-[3px] border-black rounded-xl font-black uppercase text-xs shadow-[3px_3px_0px_0px_black] hover:shadow-none hover:translate-y-0.5 transition-all">
        ⚡ Generate
       </button>
      </div>
     </div>
 ...
 return (
  <div className="max-w-sm space-y-1">
   <div>
    <button
     onClick={() => toggle("root")}
     className="flex items-center gap-2 font-black uppercase text-sm tracking-tighter hover:text-[#FF3399] transition-colors w-full text-left py-1">
     <span
      className={`transition-transform ${openNodes.includes("root") ? "rotate-90" : ""}`}>
      ▶
     </span>{" "}
     📁 Channel Assets
    </button>
    {openNodes.includes("root") && (
     <div className="ml-6 border-l-[3px] border-black/10 pl-4 space-y-1 mt-1">
      <button
       ...
       className="flex items-center gap-2 font-black uppercase text-xs tracking-tighter hover:text-[#00CCFF] transition-colors w-full text-left py-1">
       <span
        className={`transition-transform text-[10px] ${openNodes.includes("videos") ? "rotate-90" : ""}`}>
        ▶
       </span>{" "}
       🎬 Videos
      </button>
      {openNodes.includes("videos") && (
       <div className="ml-6 border-l-[2px] border-black/5 pl-3 space-y-1">
        {[
         "Episode 1 — Beach Day",
         "Episode 2 — Sunset Hike",
         "AI Tools Review (Draft)",
        ].map((v) => (
         <div
          key={v}
          className="text-xs font-bold text-black/50 py-0.5 hover:text-black transition-colors cursor-pointer">
          {v}
         </div>
        ))}
       </div>
      )}
      <div className="text-xs font-bold text-black/50 py-0.5">
       📊 Analytics Export
      </div>
      <div className="text-xs font-bold text-black/50 py-0.5">
       📝 Script Templates
...
export const BarChartDemo = () => {
 ...
 const data = [
  { label: "Mon", value: 42, color: "#FF3399" },
  { label: "Tue", value: 68, color: "#CCFF00" },
  { label: "Wed", value: 55, color: "#00CCFF" },
  { label: "Thu", value: 91, color: "#FFDD00" },
  { label: "Fri", value: 73, color: "#FFB158" },
  { label: "Sat", value: 85, color: "#FF3399" },
 ...
 return (
  <div className="border-[4px] border-black rounded-2xl bg-white shadow-[8px_8px_0px_0px_black] overflow-hidden">
   <div className="bg-[#00CCFF] border-b-[4px] border-black px-4 py-3 flex justify-between items-center">
    ...
    <span className="text-[10px] font-black uppercase tracking-widest bg-white px-3 py-1 border-[2px] border-black rounded-lg">
     7 Days
    </span>
   </div>
   <div className="p-6">
...
export const LineChartDemo = () => {
 ...
 const toPath = (d: number[]) =>
  d
   .map((v, i) => `${i === 0 ? "M" : "L"}${40 + i * 26},${150 - v * 1.3}`)
   .join(" ")
 return (
  <div className="border-[4px] border-black rounded-2xl bg-white shadow-[8px_8px_0px_0px_black] overflow-hidden">
   <div className="bg-[#CCFF00] border-b-[4px] border-black px-4 py-3 flex justify-between items-center">
    ...
    <div className="flex gap-3">
     <span className="flex items-center gap-1 text-[9px] font-black uppercase">
      <span className="w-3 h-1 bg-[#FF3399] rounded" />
      Subs
     </span>
     <span className="flex items-center gap-1 text-[9px] font-black uppercase">
      <span className="w-3 h-1 bg-[#00CCFF] rounded" />
      Views
     </span>
    </div>
   </div>
   <div className="p-6">
    <svg viewBox="0 0 350 180" className="w-full">
     ...
     {hoveredPoint !== null && (
      <text
       x={40 + hoveredPoint * 26}
       y={150 - data1[hoveredPoint] * 1.3 - 12}
       textAnchor="middle"
       fontSize="9"
       fontWeight="900"
       fill="black">
...
export const DonutChartDemo = () => {
 ...
 const slices = [
  { label: "Organic", pct: 42, color: "#CCFF00" },
  { label: "Search", pct: 28, color: "#00CCFF" },
  { label: "Suggested", pct: 18, color: "#FF3399" },
  { label: "External", pct: 12, color: "#FFDD00" },
 ]
 let cumulative = 0
 return (
   ...
   <div className="flex justify-center">
    <svg viewBox="0 0 120 120" className="w-48 h-48">
     {slices.map((s, i) => {
      ...
      return (
       <circle
        ...
        fill="none"
        stroke={s.color}
        strokeWidth={activeSlice === i ? 16 : 12}
        strokeDasharray={`${c}`}
     ...
     <text
      ...
      fill="#999">
      {activeSlice !== null
       ? slices[activeSlice].label.toUpperCase()
       : "TRAFFIC"}
     </text>
    </svg>
   </div>
   <div className="space-y-3">
    {slices.map((s, i) => (
     <div
      ...
      <div
       className="w-5 h-5 rounded-md border-[2px] border-black"
       style={{ backgroundColor: s.color }}
      />
      <span className="font-black uppercase text-xs tracking-tighter flex-1">
       {s.label}
      </span>
...
export const ScatterPlotDemo = () => {
 const [hovered, setHovered] = useState<number | null>(null)
 const points = [
  { x: 15, y: 30, s: 8, c: "#FF3399" },
  { x: 30, y: 60, s: 12, c: "#CCFF00" },
  { x: 45, y: 45, s: 6, c: "#00CCFF" },
  { x: 58, y: 80, s: 14, c: "#FFDD00" },
  { x: 70, y: 55, s: 9, c: "#FFB158" },
  { x: 82, y: 90, s: 11, c: "#FF3399" },
  { x: 25, y: 70, s: 7, c: "#00CCFF" },
  { x: 90, y: 75, s: 10, c: "#CCFF00" },
 ]
 return (
  <div className="border-[4px] border-black rounded-2xl bg-white shadow-[8px_8px_0px_0px_black] overflow-hidden">
   <div className="bg-[#FFB158] border-b-[4px] border-black px-4 py-3">
    <span className="font-[1000] uppercase tracking-tighter">
     CTR vs Watch Time
    </span>
   </div>
   <div className="p-6">
...
export const DistributionDemo = () => {
 const items = [
  { label: "Mobile", pct: 62, color: "#FF3399" },
  { label: "Desktop", pct: 22, color: "#00CCFF" },
  { label: "Tablet", pct: 10, color: "#CCFF00" },
  { label: "TV", pct: 6, color: "#FFDD00" },
 ]
 return (
  <div className="space-y-4">
   ...
   <div className="grid grid-cols-4 gap-3">
    {items.map((item, i) => (
     <div
      key={i}
      className="border-[3px] border-black rounded-xl p-3 text-center shadow-[3px_3px_0px_0px_black]"
   ...
   <div className="flex justify-center">
    <svg viewBox="0 0 120 120" className="w-44 h-44">
     ...
     <line
      x1="60"
      y1="60"
      x2={60 + 28 * Math.cos(((h * 30 + m * 0.5 - 90) * Math.PI) / 180)}
      y2={60 + 28 * Math.sin(((h * 30 + m * 0.5 - 90) * Math.PI) / 180)}
      stroke="black"
      strokeWidth="4"
      strokeLinecap="round"
     />
     <line
      x1="60"
      y1="60"
      x2={60 + 36 * Math.cos(((m * 6 - 90) * Math.PI) / 180)}
      y2={60 + 36 * Math.sin(((m * 6 - 90) * Math.PI) / 180)}
      stroke="black"
      strokeWidth="3"
      strokeLinecap="round"
     />
     <line
      x1="60"
      y1="60"
      x2={60 + 40 * Math.cos(((s * 6 - 90) * Math.PI) / 180)}
      y2={60 + 40 * Math.sin(((s * 6 - 90) * Math.PI) / 180)}
      stroke="#FF3399"
      strokeWidth="1.5"
      strokeLinecap="round"
     />
   ...
   <div className="space-y-4">
    <div className="flex gap-2">
     {[
      ...
      <React.Fragment key={i}>
       {i > 0 && (
        <span className="font-[1000] text-3xl text-[#FF3399] animate-pulse">
         :
        </span>
       )}
       <div className="bg-black text-[#CCFF00] px-4 py-3 rounded-xl border-[3px] border-black font-[1000] text-3xl tracking-tighter shadow-[4px_4px_0px_0px_#CCFF00]">
        {v}
       </div>
      </React.Fragment>
     ))}
...
export const MetersDemo = () => {
 const meters = [
  { label: "CPU Load", value: 73, color: "#FF3399" },
  { label: "Upload", value: 45, color: "#CCFF00" },
  { label: "Render", value: 88, color: "#00CCFF" },
  { label: "Storage", value: 61, color: "#FFDD00" },
 ]
 return (
  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
   {meters.map((m, i) => (
    <div
     key={i}
     className="border-[4px] border-black rounded-2xl p-4 bg-white shadow-[4px_4px_0px_0px_black] text-center space-y-3">
...
export const AvatarsDemo = () => (
 <div className="space-y-4">
  <div className="flex items-center gap-4 flex-wrap">
   {[
    { initials: "CB", color: "bg-[#FF3399]" },
    { initials: "AI", color: "bg-[#CCFF00]" },
    { initials: "VT", color: "bg-[#00CCFF]" },
    { initials: "YT", color: "bg-[#FFDD00]" },
   ].map((a, i) => (
    <div key={i} className="relative">
     <div
      className={`w-14 h-14 ${a.color} border-[3px] border-black rounded-full flex items-center justify-center font-[1000] text-lg shadow-[3px_3px_0px_0px_black]`}>
      {a.initials}
     </div>
     {i < 2 && (
      <div className="absolute -bottom-0.5 -right-0.5 w-4 h-4 bg-[#CCFF00] border-[2px] border-black rounded-full" />
     )}
    </div>
   ))}
  </div>
  <div className="flex -space-x-3">
   {[
    ...
    <div
     key={i}
     className={`w-10 h-10 ${c} border-[3px] border-black rounded-full flex items-center justify-center font-black text-xs`}
     style={{ zIndex: 5 - i }}>
     {i + 1}
    </div>
   ))}
   <div className="w-10 h-10 bg-gray-200 border-[3px] border-black rounded-full flex items-center justify-center font-black text-[10px]">
    +4
   </div>
  </div>
 </div>
  ...
  <div className="space-y-4">
   <div className="flex items-center gap-2">
    <button
     ...
     className="w-10 h-10 border-[3px] border-black rounded-xl font-black flex items-center justify-center hover:bg-[#CCFF00] transition-colors shadow-[2px_2px_0px_0px_black]">
     ◀
    </button>
    {[1, 2, 3, 4, 5].map((p) => (
     <button
      ...
      className={`w-10 h-10 border-[3px] border-black rounded-xl font-[1000] text-sm flex items-center justify-center transition-all ${
       page === p
        ? "bg-[#FF3399] text-white shadow-none scale-110"
        : "bg-white shadow-[2px_2px_0px_0px_black] hover:bg-gray-50"
      }`}>
      {p}
     </button>
    ))}
    ...
    <button
     onClick={() => setPage(12)}
     className={`w-10 h-10 border-[3px] border-black rounded-xl font-[1000] text-sm flex items-center justify-center transition-all ${page === 12 ? "bg-[#FF3399] text-white" : "bg-white shadow-[2px_2px_0px_0px_black]"}`}>
     12
    </button>
    <button
...
export const NameValueListDemo = () => {
 const items = [
  { key: "Channel", value: "ViewTUBE OS" },
  { key: "Subscribers", value: "9,340" },
  { key: "Total Videos", value: "127" },
  { key: "Created", value: "Jan 2024" },
  { key: "Niche", value: "Creator Tools" },
 ...
 return (
  <div className="border-[4px] border-black rounded-2xl overflow-hidden shadow-[4px_4px_0px_0px_black] divide-y-[2px] divide-black/10">
   {items.map((item, i) => (
    <div
     key={i}
     className="flex justify-between p-4 bg-white hover:bg-gray-50 transition-colors">
     <span className="text-[10px] font-black uppercase tracking-widest text-black/40">
      {item.key}
     </span>
     {item.badge ? (
      <span className="bg-[#CCFF00] px-3 py-0.5 border-[2px] border-black rounded-lg text-[10px] font-black uppercase">
       {item.value}
      </span>
     ) : (
  ...
  <div className="space-y-6">
   <div className="flex items-center">
    {steps.map((s, i) => (
     <React.Fragment key={i}>
      ...
      {i < steps.length - 1 && (
       <div
        className={`flex-1 h-[3px] mx-1 ${i < step ? "bg-[#CCFF00]" : "bg-black/10"}`}
       />
      )}
     </React.Fragment>
    ))}
   ...
   <div className="flex justify-between">
    {steps.map((s, i) => (
     <span
      key={i}
      className={`text-[8px] font-black uppercase tracking-widest ${i === step ? "text-[#FF3399]" : "text-black/30"}`}>
      {s}
     </span>
    ))}
   </div>
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
   <div className="bg-white border-[4px] border-black rounded-2xl p-6 shadow-[8px_8px_0px_0px_black] space-y-6">
    <div className="flex justify-between font-[1000] text-xl uppercase tracking-tighter">
     <span>Velocity</span>
     <span className="text-[#FF3399]">{velocity}%</span>
    </div>
    <input
     type="range"
     min={0}
    ...
    <div className="space-y-2">
     ...
     <div className="h-6 bg-gray-200 border-[3px] border-black rounded-full overflow-hidden">
      <div
       className="h-full bg-[#00CCFF] transition-all duration-300 rounded-full"
       style={{ width: `${storage}%` }}
      />
     </div>
     <input
    ...
    <div className="relative w-40 h-40 flex items-center justify-center">
     <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
      ...
      <circle
       ...
       strokeDashoffset={`${2 * Math.PI * 40 * (1 - velocity / 100)}`}
  ...
  <div className="relative max-w-sm w-full self-start h-fit">
   ...
   {open && (
    <div className="absolute top-full left-0 mt-3 w-full bg-white border-[4px] border-black rounded-2xl shadow-[12px_12px_0px_0px_black] overflow-hidden z-20">
     {["Launch Engine", "Analyze Data", "Export Report", "Settings"].map(
      (o, i) => (
       <button
        key={o}
        onClick={() => setOpen(false)}
        className={`w-full p-4 text-left font-black uppercase text-sm hover:bg-[#FF3399] hover:text-white transition-colors ${i < 3 ? "border-b-[3px] border-black/10" : ""}`}>
        {o}
       </button>
      ),
     )}
...
export const AreaChartDemo = () => {
 ...
 const area1 = d1
  .map((v, i) => `${pad + i * xStep},${scale(v + d2[i])}`)
  .join(" ")
 ...
 return (
  <div className="border-[4px] border-black rounded-2xl bg-white shadow-[8px_8px_0px_0px_black] overflow-hidden">
   <div className="bg-[#FF3399] border-b-[4px] border-black px-4 py-3 flex justify-between items-center">
    <span className="font-[1000] uppercase tracking-tighter text-white">
     Stacked Traffic
    </span>
    <div className="flex gap-3">
     <span className="flex items-center gap-1 text-[9px] font-black uppercase text-white/80">
      <span className="w-3 h-2 bg-[#CCFF00] rounded" />
      Organic
     </span>
     <span className="flex items-center gap-1 text-[9px] font-black uppercase text-white/80">
      <span className="w-3 h-2 bg-[#00CCFF] rounded" />
      Search
     </span>
    </div>
   </div>
