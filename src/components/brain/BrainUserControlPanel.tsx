...
export const BrainUserControlPanel: React.FC<{
 ...
 useEffect(() => {
  if (!open) return
  setControls(readBrainUserControls())
 }, [open])

 if (!open) return null

 const patch = (next: Partial<BrainUserControls>) => {
  const saved = writeBrainUserControls({ ...controls, ...next })
  setControls(saved)
  onChange?.(saved)
 }
  ...
  <div className="absolute inset-0 z-30 flex flex-col bg-white" aria-label="Brain user controls">
   <header className="flex items-center justify-between border-b-[3px] border-black bg-[#FFDA47] px-3 py-2">
    <div>
     ...
     <p className="mt-0.5 text-[8px] font-black uppercase text-black/50">Creator rules override Brain defaults</p>
    </div>
   ...
   <div className="min-h-0 flex-1 overflow-y-auto p-3">
    ...
    <Row label="Personalization" detail="Use your channel profile, goals, memory and learned patterns." value={controls.personalization} onChange={(value) => patch({ personalization: value })} />
    ...
    <Row label="Learn from interactions" detail="Allow accepted/rejected outputs and corrections to improve your personal channel model." value={controls.learnFromInteractions} onChange={(value) => patch({ learnFromInteractions: value })} />
    <Row label="Analytics access" detail="Allow Brain to use canonical VT-SYNC / analytics evidence." value={controls.allowAnalytics} onChange={(value) => patch({ allowAnalytics: value })} />
    ...
    <Row label="Publisher access" detail="Allow Brain to prepare publishing workflows." value={controls.allowPublisher} onChange={(value) => patch({ allowPublisher: value })} />
    <Row label="Confirm external actions" detail="Brain-initiated posting/publishing must stop for creator approval." value={controls.externalActionsRequireApproval} onChange={(value) => patch({ externalActionsRequireApproval: value })} />
   ...
   <footer className="grid grid-cols-2 gap-2 border-t-[3px] border-black bg-[#f7f7f3] p-2">
    <button type="button" onClick={() => patch(DEFAULT_BRAIN_USER_CONTROLS)} className="rounded-[8px] border-[2px] border-black bg-white px-2 py-2 text-[9px] font-[1000] uppercase">Reset defaults</button>
