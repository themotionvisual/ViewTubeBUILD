...
export const BrainEvaluationInboxPanel: React.FC<{
 channelId: string | null
 maximumItems?: number
 compact?: boolean
}> = ({ channelId, maximumItems = 6, compact = false }) => {
 ...
 // Canonical snapshot updates advance Phase 6 in three safe steps:
 // 1) capture genuine lifecycle evidence;
 // 2) resolve monitoring checkpoints only when matching evidence exists;
 // 3) finalize only final-horizon evaluations that preflight as measurable.
 // Missing evidence remains retryable and does not manufacture success/failure.
 useEffect(() => {
  if (!channelId) return
  const capture = () => {
   try {
 ...
 const inbox = useMemo(
  ...
  [channelId, maximumItems, revision],
 )
   ...
   <section className="overflow-hidden rounded-[10px] border-[2px] border-black bg-white" aria-label="Brain evaluation inbox">
    <div className="flex items-center justify-between gap-2 border-b-[2px] border-black bg-[#FFDA47] px-3 py-2">
     <div className="min-w-0">
      <h3 className="text-[10px] font-[1000] uppercase leading-4 tracking-[0.1em]">Evaluation Inbox</h3>
      {!compact ? (
       <p className="text-[9px] font-bold leading-3 text-black/60">
        What worked, what is overdue, and what still needs evidence.
       </p>
      ) : null}
     </div>
     <div className="flex shrink-0 items-center gap-1">
      {inbox?.counts.critical ? (
       <span className="rounded-[5px] border-[2px] border-black bg-[#FF6B6B] px-1.5 py-0.5 text-[8px] font-[1000] uppercase">
        {inbox.counts.critical} urgent
       </span>
      ) : null}
      <button
       type="button"
       title="Refresh evaluation inbox"
       aria-label="Refresh evaluation inbox"
       onClick={() => setRevision((value) => value + 1)}
       className="grid h-7 w-7 place-items-center rounded-[6px] border-[2px] border-black bg-white hover:bg-[#36E0F6]"
      >
    ...
    <div className="grid gap-1.5 p-2">
     {!items.length ? (
      <p className="rounded-[7px] border-[2px] border-black/20 bg-[#f7f7f4] p-2 text-[10px] font-bold leading-4 text-black/55">
       No evaluation work is waiting right now.
      </p>
     ) : items.map((item) => (
      <article key={item.id} className="overflow-hidden rounded-[8px] border-[2px] border-black bg-white">
       <div className="flex items-center gap-1.5 border-b-[2px] border-black px-2 py-1" style={{ backgroundColor: toneFor(item) }}>
        {iconFor(item)}
        <span className="min-w-0 flex-1 truncate text-[9px] font-[1000] uppercase tracking-[0.06em]">{item.title}</span>
        <span className="rounded-[4px] border border-black bg-white/70 px-1 text-[7px] font-black uppercase">{item.priority}</span>
       </div>
       <div className="grid gap-1.5 p-2">
        <p className="line-clamp-3 text-[10px] font-bold leading-[14px] text-black/70">{item.summary}</p>
        {item.requiredMetrics.length ? (
         <div className="flex flex-wrap gap-1">
          {item.requiredMetrics.slice(0, 4).map((metric) => (
           <span key={metric} className="rounded-[4px] border border-black px-1 py-0.5 text-[7px] font-black uppercase">{metric}</span>
          ))}
         </div>
        ) : null}
        <button
         type="button"
         onClick={() => setSelectedEventId(item.sourceEventId)}
         className="flex items-center justify-center gap-1 rounded-[5px] border-[2px] border-black bg-white px-1 py-1 text-[7px] font-black uppercase hover:bg-[#36E0F6]"
        >
         <GitBranch size={9} aria-hidden="true" /> Trace evidence
        </button>
        {item.kind === "learning_review" ? (
         <div className="grid grid-cols-3 gap-1 pt-1">
          <button type="button" onClick={() => review(item, "hold")} className="rounded-[5px] border-[2px] border-black bg-white px-1 py-1 text-[7px] font-black uppercase hover:bg-[#FFDA47]">Hold</button>
          <button type="button" onClick={() => review(item, "reject")} className="rounded-[5px] border-[2px] border-black bg-white px-1 py-1 text-[7px] font-black uppercase hover:bg-[#FF6B6B]">Reject</button>
          <button type="button" onClick={() => review(item, "approve_for_profile_review")} className="rounded-[5px] border-[2px] border-black bg-[#3FEE56] px-1 py-1 text-[7px] font-black uppercase">Review</button>
         </div>
        ) : null}
       </div>
      </article>
