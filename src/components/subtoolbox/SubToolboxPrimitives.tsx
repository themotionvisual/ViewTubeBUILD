...
export const SubToolboxKnob: React.FC<SubToolboxKnobProps> = ({
  ...
  const clamp = (next: number) => Math.min(safeMax, Math.max(min, Math.round(next / step) * step))
  ...
  const updateFromPointer = (clientX: number, clientY: number) => {
    ...
    let swept: number
    if (degrees >= 225) swept = degrees - 360
    else if (degrees <= 135) swept = degrees
    else swept = degrees < 180 ? 135 : -135
