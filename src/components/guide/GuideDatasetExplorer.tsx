  ...
  <div className="grid gap-4 lg:grid-cols-[.9fr_1.1fr]">
   <div className="rounded-2xl border-[4px] border-black bg-white p-4 shadow-[6px_6px_0_0_#000]">
    <div className="flex items-center justify-between gap-3">
     ...
     <span className="rounded-lg border-[3px] border-black bg-[#40C6E9] px-3 py-2 font-black">{GUIDE_DATASETS.length}</span>
    </div>
    <label className="mt-3 flex items-center gap-2 rounded-xl border-[3px] border-black px-3">
     ...
     <input
      ...
      className="min-w-0 flex-1 bg-transparent py-3 text-sm font-black outline-none"
     />
    </label>
    <div className="mt-3 max-h-[420px] overflow-auto rounded-xl border-[3px] border-black">
     {filtered.map((dataset) => (
      <button
       ...
       className={`block w-full border-b-2 border-black p-3 text-left last:border-b-0 ${selected?.id === dataset.id ? "bg-[#CCFF00]" : "bg-white hover:bg-[#F3F4F6]"}`}
      >
...
const DatasetDetail: React.FC<{ dataset: GuideDatasetDefinition | null }> = ({ dataset }) => {
 ...
 return (
  <article className="rounded-2xl border-[4px] border-black bg-[#40C6E9] p-5 shadow-[6px_6px_0_0_#000]">
   <div className="flex items-start justify-between gap-4">
    <Database size={30} strokeWidth={3} />
    <span className="rounded-lg border-[3px] border-black bg-white px-2 py-1 text-[10px] font-black uppercase">{dataset.id}</span>
   </div>
   <h3 className="mt-8 text-3xl font-black uppercase leading-none">{dataset.label}</h3>
   ...
   <div className="mt-3 rounded-xl border-[3px] border-black bg-white p-4">
    ...
    <div className="mt-3 flex flex-wrap gap-2">
     {dataset.categoryIds.map((category) => (
      <span key={category} className="rounded-lg border-2 border-black bg-[#F3F4F6] px-2 py-1 text-[10px] font-black uppercase">{category}</span>
     ))}
    </div>
   </div>
