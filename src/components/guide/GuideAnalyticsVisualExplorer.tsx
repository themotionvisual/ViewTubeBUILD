...
export const GuideAnalyticsVisualExplorer: React.FC = () => {
 ...
 return <div className="grid min-w-0 gap-4 lg:grid-cols-[.8fr_1.2fr]">
  <div className="min-w-0 rounded-2xl border-[4px] border-black bg-white p-4 shadow-[6px_6px_0_0_#000]">
   <div className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-black/45">Analytics source catalog</p><h3 className="text-xl font-black uppercase">Visual encyclopedia</h3></div><span className="rounded-lg border-[3px] border-black bg-[#CCFF00] px-3 py-2 font-black">{GUIDE_ANALYTICS_VISUALS.length}</span></div>
   <label className="mt-3 flex min-w-0 items-center gap-2 rounded-xl border-[3px] border-black px-3"><Search size={17} strokeWidth={3}/><input value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="Find a visual..." className="w-0 min-w-0 flex-1 bg-transparent py-3 text-sm font-black outline-none"/></label>
   <div className="mt-3 max-h-[700px] overflow-auto rounded-xl border-[3px] border-black">
    {filtered.map((v)=><button key={v.id} onClick={()=>setSelectedId(v.id)} className={`block w-full border-b-2 border-black p-3 text-left last:border-b-0 ${selected?.id===v.id?"bg-[#CCFF00]":"bg-white hover:bg-[#F3F4F6]"}`}><strong className="block text-sm font-black uppercase">{v.title}</strong><span className="mt-1 block text-[10px] font-black uppercase text-black/45">{v.family} · {v.status}</span></button>)}
   </div>
  </div>
...
const Section=({title,tone,children}:{title:string;tone:string;children:React.ReactNode})=><section className={`mt-4 rounded-xl border-[3px] border-black p-4 text-sm font-bold leading-relaxed ${tone}`}><h4 className="mb-2 text-[10px] font-black uppercase tracking-wide text-black/45">{title}</h4>{children}</section>
const List=({items}:{items:readonly string[]})=><ul className="space-y-2">{items.map(x=><li key={x} className="flex gap-2"><span>•</span><span>{x}</span></li>)}</ul>
