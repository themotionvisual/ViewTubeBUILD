...
export const GuideWidgetExplorer: React.FC = () => {
 ...
 return (
  <div className="grid min-w-0 gap-4 lg:grid-cols-[.82fr_1.18fr]">
   <div className="min-w-0 rounded-2xl border-[4px] border-black bg-white p-4 shadow-[6px_6px_0_0_#000]">
    ...
    <label className="mt-3 flex min-w-0 items-center gap-2 rounded-xl border-[3px] border-black px-3">
     <Search size={17} strokeWidth={3} />
     <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Find a widget..." className="w-0 min-w-0 flex-1 bg-transparent py-3 text-sm font-black outline-none" />
    </label>
    ...
    <div className="mt-3 max-h-[620px] overflow-auto rounded-xl border-[3px] border-black">
     {filtered.map((widget) => (
...
const WidgetDetail: React.FC<{ widget: GuideWidgetDefinition | null }> = ({ widget }) => {
 ...
 return (
  <article className="min-w-0 overflow-hidden rounded-2xl border-[4px] border-black bg-[#FF8AAF] p-4 shadow-[6px_6px_0_0_#000] sm:p-5">
   ...
   <h3 className="mt-7 break-words text-2xl font-black uppercase leading-none sm:text-3xl">{widget.title}</h3>
...
const TeachingBlock = ({ icon: Icon, title, tone, children }: { icon: React.ComponentType<{ size?: number; strokeWidth?: number }>; title: string; tone: string; children: React.ReactNode }) => (
 <section className={`mt-4 rounded-xl border-[3px] border-black p-4 ${tone}`}>
...
const TeachingList = ({ icon, title, items, tone }: { icon: React.ComponentType<{ size?: number; strokeWidth?: number }>; title: string; items: readonly string[]; tone: string }) => (
 <TeachingBlock icon={icon} title={title} tone={tone}>
  <ul className="space-y-2">
   {items.map((item) => <li key={item} className="flex gap-2 text-sm font-bold leading-relaxed"><span aria-hidden="true">•</span><span>{item}</span></li>)}
  </ul>
 </TeachingBlock>
)
