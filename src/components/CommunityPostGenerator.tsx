        ...
        <div className={toolboxSystem.shellRow}>
            <div className={toolboxSystem.inputColumn}>
                <div className="space-y-2">
                    <label className={toolboxSystem.label}>Upcoming Uploads & Plans</label>
                    <textarea
                        value={schedule}
                        onChange={(e) => setSchedule(e.target.value)}
                        placeholder="e.g. Uploading a video about Minecraft on Friday, and a Short on Tuesday..."
                        className={`${toolboxSystem.inputBase} h-40 resize-none focus:bg-[#CCFF00]/10`}
                    />
                </div>
                <button onClick={handleGenerate} disabled={loading || !schedule} className={toolboxActionButton('bg-[#00CCFF]')}>
