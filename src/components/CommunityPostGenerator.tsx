 ...
 return <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8 items-start w-full px-0 py-4 sm:py-6 lg:py-8 bg-white">
  <div className="flex flex-col gap-6 min-w-0">
   <SubToolbox title="Post Workspace" icon={<MessageSquare />} collapsible isOpenInitial>
    <div className="grid grid-cols-2 sm:grid-cols-5 gap-2" aria-label="Post type">
     {POST_TYPES.map(({ id, label, icon: Icon }) => <button key={id} type="button" aria-pressed={post.postType === id} onClick={() => post.setPostType(id)} className={`min-h-12 border-[3px] border-black rounded-[8px] px-2 py-2 font-black uppercase text-[10px] flex items-center justify-center gap-2 shadow-[3px_3px_0_0_var(--vt-subtoolbox-shadow)] active:translate-x-[3px] active:translate-y-[3px] active:shadow-none transition-[transform,box-shadow,background-color] ${post.postType === id ? "bg-[#FFE357]" : "bg-white"}`}><Icon size={15} aria-hidden="true" />{label}</button>)}
    </div>
   ...
   {post.postType.includes("poll") && <SubToolbox title="Poll Options" icon={<CheckSquare />} collapsible isOpenInitial>
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">{post.pollOptions.map((option, index) => <div key={index} className="min-w-0">
     ...
     {post.postType === "image-poll" && <><input ref={(node) => { pollInputs.current[index] = node }} className="hidden" type="file" accept="image/*" onChange={(event) => post.applyImageFile(event.target.files?.[0], index)} /><button type="button" className="mt-2 w-full min-h-11 border-[3px] border-black rounded-[8px] bg-[#73DEFF] font-black uppercase text-[9px] flex items-center justify-center gap-2 shadow-[3px_3px_0_0_var(--vt-subtoolbox-shadow)] active:translate-x-[3px] active:translate-y-[3px] active:shadow-none" onClick={() => pollInputs.current[index]?.click()}><Upload size={14} aria-hidden="true" />{post.imagePollUrls[index] ? "Replace Image" : "Add Image"}</button></>}
    </div>)}</div>
   </SubToolbox>}
  </div>
  ...
  <div className="flex flex-col gap-6 min-w-0">
   {(post.postType === "image" || post.postType === "image-poll") && <SubToolbox title="Post Media" icon={<Image />} collapsible isOpenInitial>
    ...
    <button type="button" onClick={() => imageInput.current?.click()} className="mt-3 w-full min-h-12 border-[3px] border-black rounded-[8px] bg-[#73DEFF] font-black uppercase flex items-center justify-center gap-2 shadow-[3px_3px_0_0_var(--vt-subtoolbox-shadow)] active:translate-x-[3px] active:translate-y-[3px] active:shadow-none"><Upload size={17} aria-hidden="true" />Browse Files</button>
   ...
   {post.postType === "video" && <SubToolbox title="Linked Video" icon={<Video />} collapsible isOpenInitial>
    ...
    <div className="mt-3 max-h-56 overflow-y-auto border-[3px] border-black rounded-[8px] p-2 space-y-2 shadow-[3px_3px_0_0_var(--vt-subtoolbox-shadow)]" style={{ contentVisibility: "auto" }}>{post.filteredVideos.map((video) => <button type="button" key={video.videoId} aria-pressed={post.selectedVideoId === video.videoId} onClick={() => post.setSelectedVideoId(video.videoId)} className={`w-full text-left border-[2px] border-black rounded-[6px] p-2 text-[10px] font-black ${post.selectedVideoId === video.videoId ? "bg-[#FFE357]" : "bg-white"}`}>{video.title || video.videoId}</button>)}{!post.filteredVideos.length && <p className="p-4 text-center text-[10px] font-black uppercase opacity-50">No videos found.</p>}</div>
   </SubToolbox>}

   <SubToolbox title="Post Preview" icon={<FileText />} collapsible isOpenInitial>
    <div className="min-h-48 border-[3px] border-black rounded-[8px] bg-[#FFF9E8] p-5 whitespace-pre-wrap break-words text-sm font-bold shadow-[3px_3px_0_0_var(--vt-subtoolbox-shadow)]">{post.content || <span className="opacity-35 uppercase">Your post preview will appear here.</span>}</div>
    ...
    {post.error && <div role="alert" className="border-[3px] border-black bg-[#FFB158] p-3 rounded-[8px] text-xs font-black shadow-[3px_3px_0_0_var(--vt-subtoolbox-shadow)]">{post.error}</div>}
   </SubToolbox>
   ...
   <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
    ...
    {post.channelCommunityUrl ? <a href={post.channelCommunityUrl} target="_blank" rel="noreferrer" className="min-h-14 border-[4px] border-black rounded-[8px] bg-[#86B5FF] shadow-[5px_5px_0_0_#3979DB] font-black uppercase text-lg flex items-center justify-center gap-2 active:translate-x-[4px] active:translate-y-[4px] active:shadow-none"><ExternalLink size={19} aria-hidden="true" />Go to Channel</a> : <SubToolboxGridActionButton label="Connect Channel" iconName="link" tone="blue" onClick={context.reconnect} />}
   </div>
  </div>
 </div>
}
