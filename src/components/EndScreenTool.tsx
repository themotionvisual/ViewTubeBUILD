    ...
    <div className={toolboxSystem.shellRow}>
      <div className={toolboxSystem.inputColumn}>
        <div className="space-y-2">
          <label className={toolboxSystem.label}>Video Topic</label>
          <input
            type="text"
            value={topic}
            onChange={(e) => setTopic(e.target.value)}
            placeholder="e.g. The History of Rome..."
            className={`${toolboxSystem.inputBase} focus:bg-[#CCFF00]/10`}
          />
        </div>
        <button onClick={handleGenerate} disabled={loading || !topic} className={toolboxActionButton('bg-[#CCFF00]')}>
