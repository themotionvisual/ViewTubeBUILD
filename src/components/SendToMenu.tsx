...
export const SendToMenu: React.FC<Props> = ({ packet, compact = false, onSend }) => {
 ...
 const ranked = useMemo(() => {
  const preferred = packet.suggestedTargets
   .map((id) => getViewTubeToolCapability(id))
   .filter((tool): tool is NonNullable<typeof tool> => Boolean(tool))
   .filter((tool) => compatible.some((candidate) => candidate.id === tool.id))
  const rest = compatible.filter((tool) => !preferred.some((candidate) => candidate.id === tool.id))
  return [...preferred, ...rest]
 }, [compatible, packet.suggestedTargets])
 ...
 return (
  <div className="relative inline-flex flex-col items-end font-black uppercase">
   <button
    ...
    className={`border-[3px] border-black bg-[#B7F52A] text-black shadow-[4px_4px_0_#000] active:translate-x-[2px] active:translate-y-[2px] active:shadow-[2px_2px_0_#000] ${compact ? "rounded-lg px-3 py-1.5 text-[10px]" : "rounded-xl px-4 py-2 text-xs"}`}
    aria-expanded={open}
   >
     ...
     <div className="max-h-[330px] overflow-y-auto p-2">
      {ranked.map((tool, index) => (
       <button
        type="button"
        key={tool.id}
        onClick={() => send(tool.id)}
        className="mb-2 flex w-full items-center justify-between gap-3 rounded-xl border-2 border-black bg-white px-3 py-3 text-left hover:bg-[#F5F5F5]"
       >
        <span>
         <span className="block text-[9px] tracking-[.15em] opacity-50">{index === 0 ? "BEST NEXT" : tool.kind}</span>
         <span className="block text-sm">{tool.label}</span>
         <span className="mt-0.5 block text-[10px] font-bold normal-case leading-tight opacity-60">{tool.description}</span>
        </span>
      ...
      {chains.length > 0 && (
       <div className="mt-3 border-t-2 border-black pt-3">
        <div className="px-1 pb-2 text-[9px] tracking-[.18em] opacity-50">Suggested chains</div>
        {chains.slice(0, 3).map((chain) => (
         <div key={chain.id} className="mb-2 rounded-lg bg-[#F1F1F1] px-3 py-2">
          <div className="text-[10px]">{chain.title}</div>
          <div className="mt-1 text-[9px] font-bold normal-case opacity-60">{chain.steps.map((step) => getViewTubeToolCapability(step.toolId)?.label || step.toolId).join(" → ")}</div>
         </div>
        ))}
       </div>
      )}
