...
const TitleWordNetworkCanvas: React.FC<{
 ...
 metricLabel: string
}> = ({ graph, hovered, onHover, selected, onSelect, metricLabel }) => {
 const maxViews = Math.max(...graph.nodes.map((node) => node.totalViews), 1)
 const width = 860
 const height = 340
 const instanceId = useId().replace(/[^a-zA-Z0-9_-]/g, "")
 const getNodeColor = (nodeIdx: number): string => {
  return VT_SPECTRUM_PALETTE_06[nodeIdx % VT_SPECTRUM_PALETTE_06.length]
 }
 return (
  <div className="relative h-full w-full p-8" style={{ background: "#050810" }}>
   <svg
    ...
    {graph.edges.map((edge, index) => {
      ...
      const connectionCount = graph.edges.filter(
       (candidate) => candidate.source === edge.source || candidate.target === edge.source,
      ).length
      return (
       <line
        key={index}
        ...
        x2={target.x}
        y2={target.y}
        stroke={isHovered ? "#ffffff" : `url(#${instanceId}-word-edge-grad-${index})`}
        strokeWidth={Math.max(0.6, edge.sharedVideos * 0.5 - Math.max(0, connectionCount - 1) * 0.2)}
        opacity={isSelected ? (isHovered ? 0.75 : 0.45) : 0.05}
       />
      )
     })}
    {graph.nodes.map((node, index) => {
     ...
     return (
      <g
       ...
       style={{ cursor: "pointer" }}>
       <circle cx={node.x} cy={node.y} r={radius * 2} fill={color} opacity="0.08" filter={`url(#${instanceId}-node-glow)`} />
       <circle cx={node.x} cy={node.y} r={isHovered ? radius * 1.35 : radius} fill={selected.includes(node.id) ? "#ffffff" : color} stroke={selected.includes(node.id) ? color : isHovered ? "#ffffff" : "rgba(0,0,0,0.35)"} strokeWidth={selected.includes(node.id) || isHovered ? 3 : 1} opacity="0.92" />
       <text
        x={node.x}
        y={node.y - radius - 5}
        textAnchor="middle"
   ...
   <div className="pointer-events-none absolute bottom-2 left-2 text-[9px] font-black uppercase tracking-[0.08em] text-white/40">
    Size = {metricLabel} · Click up to 5 roots · Double-click to isolate · Background resets
   </div>
  </div>
 )
}
 ...
 return (
  <SubToolboxChartModule
   ...
   header={{
    ...
    subtitle: `CO-OCCURRENCE GRAPH · NODE SIZE = ${metric.toUpperCase()} · UP TO FIVE SELECTED ROOTS`,
    icon: <CustomIcon name="analytics" size={34} />,
   }}
   ...
   footer={
    <div className="flex items-center gap-3 px-4 py-2.5">
     ...
     <span className="text-[11px] font-medium leading-5 text-white/85">
      Each bubble is a word from your video titles. Size and rank follow the selected metric. Click up to five roots to reveal their secondary network, double-click to isolate one, or click the canvas to reset.
     </span>
    </div>
   }
  >
