...
export const useCommentResponderController = (context: CreatorEngagementContext): CommentResponderController => {
 ...
 const draftReply = async () => {
  ...
  try {
   ...
   const reply = result.suggestedVideoId ? `${result.reply}\n\nCheck this out for more details: https://youtu.be/${result.suggestedVideoId}` : result.reply
 ...
 const suggestVideo = async () => {
  ...
  try {
   ...
   if (result.suggestedVideoId) setReplyTextById((current) => ({ ...current, [currentId]: `${current[currentId]?.trim() ? `${current[currentId].trim()}\n\n` : ""}You might also enjoy this related video: https://youtu.be/${result.suggestedVideoId}` }))
  } catch (cause) {
