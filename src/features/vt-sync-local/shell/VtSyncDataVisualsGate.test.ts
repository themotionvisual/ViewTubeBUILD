...
const gateSource = fs.readFileSync(new URL("./VtSyncDataVisualsGate.tsx", import.meta.url), "utf8")
...
describe("VT-SYNC visual loading boundary", () => {
 it("loads heavy visual modules only after a toolbox is opened", () => {
  ...
  expect(gateSource).toContain("{isPrimaryOpen ? (")
  expect(gateSource).toContain("{isSecondaryOpen ? (")
