...
const ALL_VT_SYNC_CATEGORY_OPTION_DEFINITIONS = [
 ...
 { id: "retention", label: "Audience Retention", description: "Watch ratios and relative retention performance for videos already present in the local catalog.", isCore: false, group: "retention", phase: "retention_sync", sourceApi: "youtube_analytics_v2", quotaClass: "high", defaultEnabled: false, canonicalTargets: ["retention_curves"] },
...
// User-facing dataset switches own exactly the categories declared by their sync unit.
// Dependency expansion remains available for internal compatibility, but must not make
// one visible dataset switch silently run another dataset/table process.
