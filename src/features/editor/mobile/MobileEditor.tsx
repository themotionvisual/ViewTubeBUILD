...
export const MobileEditor:React.FC<MobileEditorProps>=({seed,renderPreview,externalStore,layout='auto',compositionAspect:controlledAspect,onCompositionAspectChange,editorSettings})=>{
 ...
 const [localAspect,setLocalAspect]=React.useState<CompositionAspect>('portrait'); const compositionAspect=controlledAspect??localAspect; const setCompositionAspect=React.useCallback((next:CompositionAspect)=>{if(controlledAspect===undefined)setLocalAspect(next);onCompositionAspectChange?.(next)},[controlledAspect,onCompositionAspectChange]); void setCompositionAspect;
 ...
 const common={store,renderPreview,height:viewport.height,compositionAspect:aspectValue,editorSettings};
 return <div ref={rootRef} data-phone-orientation={chosen} data-composition-aspect={compositionAspect} style={{position:'relative',width:'100%',height:'100dvh',overflow:'hidden',background:'#f3f3f3',WebkitTapHighlightColor:'transparent'}}>{chosen==='portrait'?<PortraitLayout {...common}/>:<LandscapeLayout {...common}/>}</div>;
};
