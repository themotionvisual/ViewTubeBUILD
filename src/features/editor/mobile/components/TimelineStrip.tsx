 ...
 *   - Reports the real visible timeline window for the mini-map navigator.
 */
...
const Ruler: React.FC<{ pxPerSec: number; durationSec: number }> = ({ pxPerSec, durationSec }) => {
  const spacing = useMemo(() => {
    const desired = 90;
    return Math.max(0.1, Math.round((desired / pxPerSec) * 10) / 10);
  }, [pxPerSec]);
  const ticks: number[] = [];
    ...
    <div style={{ position: 'sticky', top: 0, height: HEADER_HEIGHT, background: '#0f172a', zIndex: 2, marginLeft: LABEL_WIDTH }}>
      {ticks.map((t) => (
        <div
          key={t}
          style={{
            position: 'absolute', left: t * pxPerSec, top: 0, bottom: 0, paddingLeft: 4,
            borderLeft: '1px solid #334155', color: '#94a3b8', fontSize: 10,
...
const TrackRow: React.FC<TrackRowProps> = ({
  ...
  return (
    <div style={{ position: 'absolute', top: y, left: 0, right: 0, height: TRACK_HEIGHT, display: 'flex', alignItems: 'stretch' }}>
      <div
        style={{
          position: 'sticky', left: 0, width: LABEL_WIDTH, background: '#111827', zIndex: 2,
          borderRight: '1px solid #1e293b', display: 'flex', alignItems: 'center', justifyContent: 'space-between',
          padding: '0 6px', fontSize: 10, fontWeight: 800, textTransform: 'uppercase', letterSpacing: 0.5, color: rowColor,
        }}
      >
        <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{track.name}</span>
        <div style={{ display: 'flex', gap: 3 }}>
          <button onClick={() => dispatch({ type: 'muteTrack', id: track.id })} style={miniBtn(track.muted ? '#ef4444' : '#334155')} aria-label={track.muted ? 'Unmute' : 'Mute'}>M</button>
          <button onClick={() => dispatch({ type: 'lockTrack', id: track.id })} style={miniBtn(track.locked ? '#f59e0b' : '#334155')} aria-label={track.locked ? 'Unlock' : 'Lock'}>L</button>
        </div>
      </div>
      <div
        {...rowLongPress}
        onClick={() => dispatch({ type: 'clearSelection' })}
        style={{ position: 'relative', width: totalPx, background: '#0f172a', borderBottom: '1px solid #111827', touchAction: 'pan-x pan-y' }}
      >
        {clips.map((clip) => (
          <ClipBlock key={clip.id} clip={clip} selected={state.selection.clipIds.includes(clip.id)} color={rowColor} pxPerSec={pxPerSec} store={store} onContextMenu={onClipContextMenu} />
        ))}
      </div>
    </div>
  );
...
const miniBtn = (bg: string): React.CSSProperties => ({
  width: 18, height: 18, borderRadius: 4, border: 'none', background: bg, color: '#fff', fontSize: 9, fontWeight: 800, cursor: 'pointer',
});
...
const ClipBlock: React.FC<ClipBlockProps> = ({ clip, selected, color, pxPerSec, store, onContextMenu }) => {
  ...
  return (
    <div
      ...
      style={{
        position: 'absolute', top: 4, bottom: 4, left, width, borderRadius: 8,
        ...
        color: '#0f172a', padding: '4px 8px', fontSize: 11, fontWeight: 800,
        overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', touchAction: 'none', userSelect: 'none',
      }}
    >
...
const TrimHandle: React.FC<{ side: 'left' | 'right'; clip: VtE1Clip; store: EditorStore; pxPerSec: number }> = ({ side, clip, store, pxPerSec }) => {
  ...
  return (
    <div {...drag} onClick={(ev) => ev.stopPropagation()} style={{ position: 'absolute', top: 0, bottom: 0, [side]: 0, width: 16, cursor: 'ew-resize', touchAction: 'none' }}>
      <div style={{ position: 'absolute', top: '50%', [side]: 3, transform: 'translateY(-50%)', width: 4, height: 16, borderRadius: 2, background: 'rgba(15,23,42,0.55)' }} />
    </div>
  );
};
...
const Playhead: React.FC<{ playheadSec: number; pxPerSec: number; height: number }> = ({ playheadSec, pxPerSec, height }) => {
  ...
  return (
    <div style={{ position: 'absolute', left: x, top: HEADER_HEIGHT - 6, height: height + 12, width: 2, background: '#f43f5e', pointerEvents: 'none', boxShadow: '0 0 6px rgba(244,63,94,0.7)' }}>
...
const ZoomControls: React.FC<{ pxPerSec: number; onZoom: (v: number) => void }> = ({ pxPerSec, onZoom }) => (
  <div style={{ position: 'absolute', top: 4, right: 6, zIndex: 3, display: 'flex', gap: 4, background: 'rgba(15,23,42,0.85)', borderRadius: 8, padding: 3 }}>
    <button onClick={() => onZoom(pxPerSec / 1.4)} style={{ width: 26, height: 22, borderRadius: 5, border: 'none', background: '#334155', color: '#fff', fontWeight: 800 }} aria-label="Zoom out timeline">−</button>
    <div style={{ color: '#cbd5e1', fontSize: 10, alignSelf: 'center', minWidth: 40, textAlign: 'center', fontVariantNumeric: 'tabular-nums' }}>{Math.round(pxPerSec)}px/s</div>
    <button onClick={() => onZoom(pxPerSec * 1.4)} style={{ width: 26, height: 22, borderRadius: 5, border: 'none', background: '#334155', color: '#fff', fontWeight: 800 }} aria-label="Zoom in timeline">+</button>
  </div>
);
