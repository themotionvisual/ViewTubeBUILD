    ...
    >
      <div style={{position:'absolute',left:0,right:0,top:0,height:CLIP_BODY_HEIGHT}}>
        ...
        {!readOnly?(state.project.transitions??[]).filter(transition=>{
          ...
          pxPerSec={pxPerSec}
          selectedId={state.selection.transitionId}
          onSelect={id=>dispatch({type:'selectTransition',id})}
        />):null}
      </div>
...
const TransitionChip:React.FC<{
  transition:NonNullable<EditorStore['state']['project']['transitions']>[number];
  clips:VtE1Clip[];
  pxPerSec:number;
  selectedId?:string;
  ...
  const seam=(left.end+right.start)/2;
  const duration=Math.max(.12,Number(transition.durationSec??.4));
  const width=Math.max(18,duration*pxPerSec);
  ...
  return <button
    ...
    aria-label={`Select transition ${label}`}
    onPointerDown={event=>event.stopPropagation()}
    onClick={event=>{event.stopPropagation();onSelect(id)}}
    style={{
      position:'absolute',left:seam*pxPerSec-width/2,top:2,width,height:22,zIndex:8,
      border:`2px solid ${selected?CYAN:INK}`,borderRadius:4,
      background:YELLOW,color:'#111',padding:'0 3px',fontSize:6,fontWeight:1000,
      textTransform:'uppercase',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',
      boxShadow:selected?'0 0 0 2px #fff,2px 2px 0 rgba(36,139,153,.35)':'2px 2px 0 rgba(36,139,153,.25)',
      ...IOS_TOUCH_SAFE,
    }}
  >{label}</button>;
};
