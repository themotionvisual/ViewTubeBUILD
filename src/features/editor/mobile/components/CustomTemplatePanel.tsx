  ...
  return <div style={{width:'100%',minWidth:0,overflowX:'hidden'}}>
    ...
    <section style={card}>
      ...
      {!clip||!template?<div style={{fontSize:9,fontWeight:800,opacity:.6}}>Add or select a design-template clip to customize its text, icons, and colors.</div>:<>
        ...
        <div
          onContextMenu={event=>{
            const node=(event.target as HTMLElement).closest?.('[data-template-element-id]') as HTMLElement|null;
            const element=editables.find(item=>item.id===node?.dataset.templateElementId);
            if(!element)return;
          ...
          {editables.map(element=>{
            ...
            return <span
              ...
              style={{
                ...
                transform:`rotate(${Number(element.rotation??0)}deg)`,transformOrigin:'top left',pointerEvents:'none',
              }}
            />;
          })}
        </div>
