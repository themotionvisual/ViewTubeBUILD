...
import {VT_E1_FX_CATALOG,VT_E1_DEFAULT_FX_ORDER,normalizeVtE1FxDisabled,normalizeVtE1FxOrder,resetVtE1FxPatch,vtE1FxValue} from '../../../../shared/vtE1FxCatalog.js';
...
const FX_DEFS=VT_E1_FX_CATALOG;
const DEFAULT_FX_ORDER=VT_E1_DEFAULT_FX_ORDER;
...
export function ClipEffects({store}:{store:EditorStore}){
  ...
  const number=(key:string,fallback:number)=>vtE1FxValue(payload,key as Parameters<typeof vtE1FxValue>[1])??fallback;
  const bypass=Boolean(payload.fxBypass);
  const disabled=normalizeVtE1FxDisabled(payload.fxDisabled) as Record<string,boolean>;
