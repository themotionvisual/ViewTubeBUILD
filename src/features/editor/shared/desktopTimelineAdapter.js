 ...
 *
 * VT_E1.jsx keeps ownership of React state, history, selection, gestures,
 * feature flags and UI. This module only normalizes calls into the shared
 * timeline math so desktop and mobile can converge without a reducer swap.
 */
export const desktopTimelineAdapter = Object.freeze({
  split(clips, clipId, playheadSec, makeRightId) {
    ...
    const right = {
      ...split.right,
      id: typeof makeRightId === 'function'
        ? makeRightId(clip)
        : `${clip.id}_r_${Date.now().toString(36)}`,
    };
    return {
      clips: clips.flatMap((entry) => entry.id === clipId ? [split.left, right] : [entry]),
      changed: true,
      left: split.left,
      right,
    };
  ...
  slip(clips, clipId, deltaSec, sourceDurationSec = Number.POSITIVE_INFINITY) {
    ...
    return {
      clips: clips.map((entry) => entry.id === clipId ? slipped : entry),
      changed: true,
      clip: slipped,
    };
  },
  ...
  slide(clips, clipId, deltaSec) {
    ...
    return {
      clips: result.clips,
      changed: Boolean(result.appliedDeltaSec),
      appliedDeltaSec: result.appliedDeltaSec,
  ...
  rippleDelete(clips, clipIds) {
    ...
    return {
      clips: rippleDeleteTimelineClips(clips, clipIds),
      changed: true,
    };
  },
});
