        ...
        <header className="bg-[#00ccff] h-[80px] border-b-[5px] border-black flex items-center justify-between px-0 overflow-hidden">
          <div className="flex items-center h-full">
            <div className="bg-black h-full w-[80px] flex items-center justify-center border-r-[5px] border-black flex-shrink-0">
              <Database size={40} strokeWidth={3} className="text-[#00ccff]" />
            </div>
            <h1 className="text-[50px] font-[1000] uppercase tracking-tighter text-black pl-8 leading-none">PROJECT VAULT</h1>
          </div>
        </header>
          ...
          <div className="flex flex-col lg:flex-row gap-8 flex-1 overflow-hidden">
            ...
            <div className="w-full lg:w-72 flex flex-col gap-4">
              <div className="bg-white border-[4px] border-black rounded-3xl p-6 shadow-[6px_6px_0px_0px_black]">
                <h3 className="text-xs font-black uppercase tracking-widest text-black/50 mb-4 px-2">Active Project</h3>
                <div className="space-y-3">
                  {['General', ...brain.projects.map(p => p.name)].map((p) => (
                    <button
                      key={p}
                      onClick={() => setSelectedProject(p)}
                      className={`w-full text-left p-4 rounded-2xl border-[3px] border-black font-black uppercase tracking-tighter transition-all flex items-center justify-between group ${selectedProject === p ? 'bg-black text-[#ccff00] translate-x-1' : 'bg-white text-black hover:bg-gray-50 hover:translate-x-1'}`}
                    >
              ...
              <div className="bg-[#ccff00] border-[4px] border-black rounded-3xl p-6 shadow-[6px_6px_0px_0px_black] group">
                <h4 className="font-black uppercase tracking-tighter text-black mb-2 flex items-center gap-2">
            ...
            {/* Main: Asset Grid */}
            <div className="flex-1 flex flex-col bg-white border-[4px] border-black rounded-[28px] shadow-[8px_8px_0px_0px_black] overflow-hidden">
              <div className="p-6 border-b-[4px] border-black flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-2xl font-[1000] uppercase tracking-tighter leading-none">{selectedProject}</h2>
                ...
                <button
                  onClick={fetchAssets}
                  disabled={loading}
                  className="bg-black text-white px-6 py-3 rounded-2xl font-black uppercase tracking-tighter border-[3px] border-black shadow-[4px_4px_0px_0px_#ccff00] hover:translate-x-1 hover:translate-y-1 hover:shadow-none transition-all flex items-center gap-3 disabled:opacity-50"
                >
              ...
              <div className="flex-1 overflow-y-auto p-8">
                {assets.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-black/20 space-y-4">
                    ...
                    <p className="font-black uppercase text-xl tracking-tighter">No assets detected for this protocol.</p>
                  </div>
                ) : (
