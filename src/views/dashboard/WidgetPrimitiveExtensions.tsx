...
const primitiveClass = (height:WidgetControlHeight,tone:WidgetPrimitiveTone) => widgetSizedControlClasses(height,tone)

export const WidgetSizedButton:React.FC<React.ButtonHTMLAttributes<HTMLButtonElement>&{height?:WidgetControlHeight;tone?:WidgetPrimitiveTone}> = ({height=32,tone="default",className="",type="button",...props}) => <button type={type} className={`vt-button vt-interactive ${primitiveClass(height,tone)} ${className}`.trim()} {...props}/>
export const WidgetLeftSplitButton:React.FC<Omit<React.ButtonHTMLAttributes<HTMLButtonElement>,"children">&{icon:React.ReactNode;children:React.ReactNode;tone?:WidgetPrimitiveTone;iconStyle?:WidgetSplitIconStyle;width?:"auto"|"compact"|"wide"|"full";height?:WidgetControlHeight}> = ({icon,children,tone="default",iconStyle="white-on-color",width="auto",height=32,className="",...props}) => <WidgetSplitButton icon={icon} tone="neutral" width={width} className={`is-left-split ${primitiveClass(height,tone)} is-icon-${iconStyle} ${className}`.trim()} {...props}>{children}</WidgetSplitButton>
export const WidgetTextInput:React.FC<React.InputHTMLAttributes<HTMLInputElement>&{height?:WidgetControlHeight;tone?:WidgetPrimitiveTone}> = ({height=32,tone="default",className="",...props}) => <input className={`vt-input widget-text-input ${primitiveClass(height,tone)} ${className}`.trim()} {...props}/>
export const WidgetSizedSelect:React.FC<{value:string;onChange:(value:string)=>void;options:WidgetSelectOption[];label:string;placeholder?:string;disabled?:boolean;className?:string;style?:React.CSSProperties;height?:WidgetControlHeight;tone?:WidgetPrimitiveTone}> = ({height=32,tone="default",className="",...props}) => <WidgetSelect className={`${primitiveClass(height,tone)} ${className}`.trim()} contentClassName={primitiveClass(height,tone)} {...props}/>
...
export const WidgetSpectrumFillBadge:React.FC<{children:React.ReactNode;spectrum:WidgetBadgeSpectrumName;height?:WidgetControlHeight;className?:string}> = ({children,spectrum,height=24,className=""}) => <span className={`widget-spectrum-fill-badge ${widgetControlHeightClass(height)} is-spectrum-${spectrum} ${className}`.trim()}>{children}</span>
