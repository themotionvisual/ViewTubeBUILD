import { getDashboardWidgetPaletteColors } from "../../styles/toolboxPalette"
import { assertUniqueIds } from "../../services/registryAssertions"
import { HEIGHT_BUCKET_ORDER, SIZE_BUCKET_ORDER } from "./tokens"
import type {
 DashboardHeightBucket,
 DashboardSizeBucket,
 WidgetDefinition,
 WidgetDefinitionBase,
} from "./types"

type DefaultDashboardSlot = Readonly<{
 id: string
 size: DashboardSizeBucket
 height: DashboardHeightBucket
}>

export const DEFAULT_DASHBOARD_ROWS = [
 // ── brain-owner-consent parity (screenshot canon) ─────────────────────
 [
  { id: "app-verification-explainer", size: "third", height: "tall" },
  { id: "kpi-cluster", size: "third", height: "tall" },
  { id: "overview-data-visuals", size: "third", height: "tall" },
 ],
 [
  { id: "comment-replier", size: "full", height: "tall" },
 ],
 [
  { id: "consistency-heatmap", size: "quarter", height: "medium" },
  { id: "realtime-performance", size: "quarter", height: "medium" },
  { id: "goals-tracker", size: "quarter", height: "medium" },
  { id: "keyword-engine", size: "quarter", height: "medium" },
 ],
 [
  { id: "daily-oracle", size: "half", height: "xtall" },
  { id: "community-post", size: "half", height: "xtall" },
 ],
 [
  { id: "image-generator", size: "half", height: "xtall" },
  { id: "data-edit", size: "half", height: "xtall" },
 ],
 [
  { id: "traffic-sources", size: "quarter", height: "medium" },
  { id: "shorts-vs-long", size: "quarter", height: "medium" },
  { id: "publish-momentum", size: "quarter", height: "medium" },
  { id: "audience-matrix", size: "quarter", height: "medium" },
 ],
 [
  { id: "system-micro-stack", size: "quarter", height: "tall" },
  { id: "keyword-overlap-intelligence", size: "quarter", height: "tall" },
  { id: "retention-sim", size: "quarter", height: "tall" },
  { id: "upload-scheduler", size: "quarter", height: "tall" },
 ],
 [
  { id: "brain-hub", size: "half", height: "xtall" },
  { id: "thumb-ai", size: "half", height: "xtall" },
 ],
 [
  { id: "quick-actions", size: "half", height: "medium" },
  { id: "revenue-momentum", size: "half", height: "medium" },
 ],
 [
  { id: "title-rewriter", size: "quarter", height: "medium" },
  { id: "description-editor", size: "quarter", height: "medium" },
  { id: "hashtag-analyzer", size: "quarter", height: "medium" },
  { id: "tag-generator", size: "quarter", height: "medium" },
 ],
 [
  { id: "superfan-card", size: "half", height: "xtall" },
  { id: "flight-check", size: "half", height: "xtall" },
 ],
 [
  { id: "channel-overview", size: "half", height: "tall" },
  { id: "audience-retention", size: "half", height: "tall" },
 ],
 [
  { id: "relative-retention-benchmark", size: "half", height: "tall" },
  { id: "ad-stack-intelligence", size: "half", height: "tall" },
 ],
 [
  { id: "revenue-chart", size: "half", height: "tall" },
  { id: "recent-uploads", size: "half", height: "tall" },
 ],
 [
  { id: "top-performer", size: "full", height: "medium" },
 ],
 [
  { id: "alerts-ticker", size: "full", height: "medium" },
 ],
 [
  { id: "burnout-monitor", size: "half", height: "tall" },
  { id: "ui-reference-library", size: "half", height: "tall" },
 ],
 [
  { id: "sync-controller", size: "full", height: "tall" },
 ],
 [
  { id: "video-uploader", size: "half", height: "xtall" },
  { id: "script-studio", size: "half", height: "xtall" },
 ],
 [
  { id: "dashboard-controls", size: "half", height: "tall" },
  { id: "account-billing",    size: "half", height: "tall" },
 ],
 [
  { id: "alerts-feed", size: "full", height: "tall" },
 ],
 [
  { id: "bridge-efficiency", size: "half", height: "medium" },
  { id: "collab-matchmaker", size: "half", height: "medium" },
 ],
 [
  { id: "mini-calendar", size: "half", height: "medium" },
  { id: "reach-funnel", size: "half", height: "medium" },
 ],
 [
  { id: "task-stack", size: "half", height: "medium" },
  { id: "video-autopsy", size: "half", height: "medium" },
 ],
 [
  { id: "daily-command-center", size: "third", height: "tall" },
  { id: "opportunity-desk",     size: "third", height: "tall" },
  { id: "idea-portfolio",       size: "third", height: "tall" },
 ],
 [
  { id: "community-post-studio", size: "half", height: "xtall" },
  { id: "shorts-generator",      size: "half", height: "xtall" },
 ],
 [
  { id: "brain-control-center", size: "full", height: "xtall" },
 ],
] as const satisfies readonly (readonly DefaultDashboardSlot[])[]

/**
 * Widgets that stay registered (findable via the Widgets tab picker) but
 * are HIDDEN from the default layout because they are obviously unfinished.
 * The user can restore any of these from the picker at any time.
 *
 * Audit: 2026-09-01 · flagged as thin stubs, placeholder charts, or
 * hardcoded/mock output. The 6 kept-and-rebuilt tools live below in
 * INITIAL_BETA_IDS.
 */
export const INITIAL_HIDDEN_IDS = new Set<string>([
  "alerts-feed",
  "alerts-ticker",
  "bridge-efficiency",
  "channel-overview",
  "task-stack",
  "top-performer",
  "ad-stack-intelligence",
  "relative-retention-benchmark",
  "reach-funnel",
  "superfan-card",
  "burnout-monitor",
  "system-micro-stack",
  "ui-reference-library",
  "retention-sim",
])

/** Kept-and-rebuilt tools shipped as "beta". Visible by default, marked as
 *  beta in the widget shell/picker so users know they're evolving. */
export const INITIAL_BETA_IDS = new Set<string>([
  "collab-matchmaker",
  "video-autopsy",
  "mini-calendar",
  "quick-actions",
  "hashtag-analyzer",
  "keyword-overlap-intelligence",
])

const defaultDashboardSlotById = new Map<string, DefaultDashboardSlot>(
 DEFAULT_DASHBOARD_ROWS.flat().map((slot) => [slot.id, slot]),
)

const DASHBOARD_WIDGET_BASE_REGISTRY: WidgetDefinitionBase[] = [
 {
  id: "dashboard-controls",
  title: "Dashboard Controls",
  subtitle: "Layout management, widget picker, preset configs, and backup tools",
  category: "system",
  defaultSize: "half",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "tall",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(0),
  dependency: ["none"],
  status: "ready",
 },
 {
  id: "app-verification-explainer",
  title: "About VIEWTUBE",
  subtitle: "VIEWTUBE app purpose, Google sign-in, and YouTube data use",
  category: "system",
  defaultSize: "third",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "medium",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(7),
  dependency: ["none"],
  status: "ready",
 },
  {
  id: "kpi-cluster",
  title: "Channel Overview",
  subtitle: "Subscribers, views, hours, revenue, CTR, AVD",
  category: "core",
  defaultSize: "half",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "tall",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(0),
  dependency: ["youtube_analytics_v2"],
  status: "ready",
 },
 {
  id: "account-billing",
  title: "Account & Billing",
  subtitle: "Profile, YouTube connection, plan, AI credits",
  category: "system",
  defaultSize: "third",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "tall",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(3),
  dependency: ["none"],
  status: "ready",
 },
 {
  id: "overview-data-visuals",
  title: "Data Visuals",
  subtitle: "Format donuts, revenue bars, discovery area, performance trend",
  category: "core",
  defaultSize: "half",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "tall",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(4),
  dependency: ["youtube_analytics_v2"],
  status: "ready",
 },
 {
  id: "channel-overview",
  title: "Social Channels",
  subtitle: "Cross-platform follower counts",
  category: "core",
  defaultSize: "quarter",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "medium",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(1),
  dependency: ["youtube_data_v3"],
  status: "ready",
 },
 {
  id: "mini-calendar",
  title: "Mini Calendar",
  subtitle: "2-week calendar heat",
  category: "core",
  defaultSize: "half",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "medium",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(2),
  dependency: ["none"],
  status: "ready",
 },
 {
  id: "task-stack",
  title: "Task Stack",
  subtitle: "Today queue",
  category: "core",
  defaultSize: "quarter",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "medium",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(3),
  dependency: ["none"],
  status: "ready",
 },
 {
  id: "quick-actions",
  title: "Quick Actions",
  subtitle: "Primary navigation launchers",
  category: "core",
  defaultSize: "full",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "medium",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(4),
  dependency: ["none"],
  status: "ready",
 },
 {
  id: "recent-uploads",
  title: "Recent Uploads",
  subtitle: "Latest 3 videos",
  category: "analytics",
  defaultSize: "third",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "medium",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(5),
  dependency: ["youtube_data_v3", "youtube_analytics_v2"],
  status: "ready",
 },
 {
  id: "top-performer",
  title: "Top Performer",
  subtitle: "Best view leader",
  category: "analytics",
  defaultSize: "third",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "medium",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(6),
  dependency: ["youtube_analytics_v2"],
  status: "ready",
 },
 {
  id: "alerts-feed",
  title: "Alerts Feed",
  subtitle: "Operational highlights",
  category: "system",
  defaultSize: "half",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "medium",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(7),
  dependency: ["none"],
  status: "ready",
 },
 {
  id: "revenue-momentum",
  title: "Revenue Momentum",
  subtitle: "4-week pulse",
  category: "analytics",
  defaultSize: "third",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "medium",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(9),
  dependency: ["youtube_analytics_v2"],
  status: "ready",
 },
 {
  id: "superfan-card",
  title: "Superfan Card",
  subtitle: "Community spotlight",
  category: "community",
  defaultSize: "half",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "medium",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(10),
  dependency: ["none"],
  status: "prototype",
 },
 {
  id: "system-micro-stack",
  title: "Settings",
  subtitle: "System, sync & mode controls",
  category: "system",
  defaultSize: "quarter",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "medium",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(11),
  dependency: ["none"],
  status: "ready",
 },
 {
  id: "consistency-heatmap",
  title: "Upload Cadence",
  subtitle: "30-day activity heat",
  category: "analytics",
  defaultSize: "quarter",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "medium",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(12),
  dependency: ["youtube_data_v3"],
  status: "ready",
 },
 {
  id: "goals-tracker",
  title: "Goals Tracker",
  subtitle: "Milestones and targets",
  category: "core",
  defaultSize: "quarter",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "medium",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(13),
  dependency: ["none"],
  status: "ready",
 },
 {
  id: "alerts-ticker",
  title: "News Ticker",
  subtitle: "Real-time system updates",
  category: "system",
  defaultSize: "full",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "medium",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(14),
  dependency: ["none"],
  status: "ready",
 },
 {
  id: "tag-generator",
  title: "Tag Generator",
  subtitle: "AI SEO tag generator",
  category: "creation",
  defaultSize: "half",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "medium",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(15),
  dependency: ["youtube_data_v3", "gemini_api"],
  status: "ready",
 },
 {
  id: "revenue-chart",
  title: "Revenue Tracker",
  subtitle: "Weekly grouped ad revenue",
  category: "analytics",
  defaultSize: "half",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "medium",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(16),
  dependency: ["youtube_analytics_v2"],
  status: "ready",
 },
 {
  id: "community-post",
  title: "Community Post",
  subtitle: "Draft and create posts",
  category: "creation",
  defaultSize: "third",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "tall",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(17),
  dependency: ["youtube_data_v3", "gemini_api"],
  status: "ready",
 },
 {
  id: "thumb-ai",
  title: "Thumb AI",
  subtitle: "Generate and analyze thumbnails",
  category: "creation",
  defaultSize: "half",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "medium",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(18),
  dependency: ["gemini_api"],
  status: "ready",
 },
 {
  id: "description-editor",
  title: "Description Editor",
  subtitle: "Write and template video descriptions",
  category: "creation",
  defaultSize: "half",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "medium",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(19),
  dependency: ["none"],
  status: "ready",
 },
 {
  id: "script-studio",
  title: "Script Studio",
  subtitle: "AI script editor with runtime estimation and voice matching",
  category: "creation",
  defaultSize: "half",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "tall",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(20),
  dependency: ["none"],
  status: "ready",
 },
 {
  id: "realtime-performance",
  title: "Realtime",
  subtitle: "Stats from last 48hr and 60m",
  category: "analytics",
  defaultSize: "quarter",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "medium",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(20),
  dependency: ["none"],
  status: "ready",
 },
 {
  id: "keyword-engine",
  title: "Keyword Engine",
  subtitle: "Top keywords by view yield",
  category: "analytics",
  defaultSize: "quarter",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "medium",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(21),
  dependency: ["none"],
  status: "ready",
 },
 {
  id: "keyword-overlap-intelligence",
  title: "Keyword Overlap",
  subtitle: "Venn map of title keyword value",
  category: "analytics",
  defaultSize: "half",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "tall",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(22),
  dependency: ["none"],
  status: "ready",
 },
 {
  id: "publish-momentum",
  title: "Published Momentum",
  subtitle: "2D Heatmap of publish times",
  category: "analytics",
  defaultSize: "half",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "medium",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(23),
  dependency: ["none"],
  status: "ready",
 },
 {
  id: "traffic-sources",
  title: "Traffic Sources",
  subtitle: "Origin of viewership",
  category: "analytics",
  defaultSize: "half",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "medium",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(24),
  dependency: ["none"],
  status: "ready",
 },
 {
  id: "audience-retention",
  title: "Retention Dip",
  subtitle: "Audience hold graph",
  category: "analytics",
  defaultSize: "half",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "medium",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(25),
  dependency: ["none"],
  status: "ready",
 },
 {
  id: "shorts-vs-long",
  title: "Long vs Short",
  subtitle: "Shorts vs Long comparative",
  category: "analytics",
  defaultSize: "quarter",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "medium",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(26),
  dependency: ["none"],
  status: "ready",
 },
 {
  id: "comment-replier",
  title: "Comment Responder",
  subtitle: "View, reply & provoke engagement",
  category: "community",
  defaultSize: "third",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "tall",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(27),
  dependency: ["youtube_data_v3", "gemini_api"],
  status: "ready",
 },
 {
  id: "reach-funnel",
  title: "Reach Funnel",
  subtitle: "Impressions to CTR conversion",
  category: "analytics",
  defaultSize: "half",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "medium",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(28),
  dependency: ["youtube_analytics_v2"],
  status: "ready",
 },
 {
  id: "relative-retention-benchmark",
  title: "Algo Benchmark",
  subtitle: "Relative performance vs Platform",
  category: "analytics",
  defaultSize: "half",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "medium",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(29),
  dependency: ["youtube_analytics_v2"],
  status: "ready",
 },
 {
  id: "ad-stack-intelligence",
  title: "The Ad Stack",
  subtitle: "Gross Rev, Monetized Playbacks, CPM",
  category: "analytics",
  defaultSize: "half",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "medium",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(30),
  dependency: ["youtube_analytics_v2"],
  status: "ready",
 },
 {
  id: "audience-matrix",
  title: "Audience Matrix",
  subtitle: "Geo, Devices, Traffic, & Sharing",
  category: "community",
  defaultSize: "full",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "medium",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(31),
  dependency: ["youtube_analytics_v2"],
  status: "ready",
 },
 {
  id: "bridge-efficiency",
  title: "Bridge Efficiency",
  subtitle: "Impression to Interaction Funnel",
  category: "analytics",
  defaultSize: "half",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "medium",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(32),
  dependency: ["youtube_analytics_v2"],
  status: "ready",
 },
 {
  id: "daily-oracle",
  title: "Daily Command Center",
  subtitle: "Oracle-powered daily priorities, ranked actions, and Brain feedback loop",
  category: "ai",
  defaultSize: "third",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "xtall",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(34),
  dependency: ["none"],
  status: "ready",
 },
 {
  id: "flight-check",
  title: "Upload Flight Check",
  subtitle: "Pre-publish checklist",
  category: "creation",
  defaultSize: "half",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "medium",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(35),
  dependency: ["none"],
  status: "ready",
 },
 {
  id: "image-generator",
  title: "Image Generator",
  subtitle: "Thumbnail + end screen image generation",
  category: "creation",
  defaultSize: "half",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "xtall",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(36),
  dependency: ["gemini_api"],
  status: "ready",
 },
 {
  id: "video-uploader",
  title: "Video Uploader",
  subtitle: "Upload and publish a new video",
  category: "creation",
  defaultSize: "half",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "xtall",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(37),
  dependency: ["youtube_data_v3"],
  status: "ready",
 },
 {
  id: "data-edit",
  title: "Video Manager",
  subtitle: "Edit published video metadata",
  category: "creation",
  defaultSize: "half",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "xtall",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(37),
  dependency: ["none"],
  status: "ready",
 },
 {
  id: "title-rewriter",
  title: "Title Rewriter",
  subtitle: "AI title alternatives",
  category: "creation",
  defaultSize: "third",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "medium",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(38),
  dependency: ["none"],
  status: "ready",
 },
 {
  id: "retention-sim",
  title: "Retention Simulator",
  subtitle: "Analyze danger zones",
  category: "analytics",
  defaultSize: "third",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "medium",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(39),
  dependency: ["none"],
  status: "ready",
 },
 {
  id: "upload-scheduler",
  title: "Upload Scheduler",
  subtitle: "7-day content planner",
  category: "creation",
  defaultSize: "third",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "medium",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(40),
  dependency: ["none"],
  status: "ready",
 },
 {
  id: "hashtag-analyzer",
  title: "Hashtag Analyzer",
  subtitle: "Tag reach and competition",
  category: "analytics",
  defaultSize: "third",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "medium",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(41),
  dependency: ["none"],
  status: "ready",
 },
 {
  id: "burnout-monitor",
  title: "Burnout Monitor",
  subtitle: "Health and pacing tracker",
  category: "system",
  defaultSize: "half",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "medium",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(42),
  dependency: ["none"],
  status: "ready",
 },
  {
   id: "collab-matchmaker",
   title: "Collab Matchmaker",
   subtitle: "Find peers and generate pitches",
   category: "community",
   defaultSize: "half",
   minSize: "twenty-fourth",
   maxSize: "full",
   defaultHeight: "medium",
   minHeight: "short",
   maxHeight: "boundless",
   ...getDashboardWidgetPaletteColors(43),
   dependency: ["gemini_api", "youtube_data_v3"],
   status: "ready",
  },
  {
    id: "brain-hub",
    title: "Central Intelligence Hub",
    subtitle: "Ask AI, Creator Journal, Brain OS & Strategy Prompts",
    category: "ai",
    defaultSize: "half",
    minSize: "twenty-fourth",
    maxSize: "full",
    defaultHeight: "xtall",
    minHeight: "short",
    maxHeight: "boundless",
    ...getDashboardWidgetPaletteColors(45),
    dependency: ["none"],
    status: "ready",
  },
  {
   id: "ui-reference-library",
   title: "UI Reference Library",
   subtitle: "Component library and scrollbar styles",
   category: "system",
   defaultSize: "half",
   minSize: "twenty-fourth",
   maxSize: "full",
   defaultHeight: "xtall",
   minHeight: "short",
   maxHeight: "boundless",
   ...getDashboardWidgetPaletteColors(7),
   dependency: ["none"],
   status: "ready",
  },
  {
    id: "video-autopsy",
    title: "Video Autopsy",
    subtitle: "Deep performance diagnosis",
    category: "analytics",
    defaultSize: "half",
    minSize: "twenty-fourth",
    maxSize: "full",
    defaultHeight: "xtall",
    minHeight: "short",
    maxHeight: "boundless",
    headerColor: "#FF1744",
    iconRailColor: "#C9F830",
    dependency: ["youtube_analytics_v2", "gemini_api"],
    status: "ready",
  },
  {
    id: "sync-controller",
    title: "Sync Controller",
    subtitle: "Monitor, trigger, and diagnose every VT-SYNC dataset",
    category: "system",
    defaultSize: "half",
    minSize: "twenty-fourth",
    maxSize: "full",
    defaultHeight: "tall",
    minHeight: "short",
    maxHeight: "boundless",
    headerColor: "#36E0F6",
    iconRailColor: "#C0F240",
    dependency: ["youtube_analytics_v2"],
    status: "ready",
  },
 {
  id: "daily-command-center",
  title: "Daily Command Center",
  subtitle: "Priority engine, focus timer, action queue, creator brief",
  category: "core",
  defaultSize: "third",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "tall",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(16),
  dependency: ["none"],
  status: "ready",
 },
 {
  id: "opportunity-desk",
  title: "Opportunity Desk",
  subtitle: "Ranked topic candidates with evidence and confidence",
  category: "core",
  defaultSize: "third",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "tall",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(17),
  dependency: ["youtube_data_v3"],
  status: "ready",
 },
 {
  id: "idea-portfolio",
  title: "Idea Portfolio",
  subtitle: "Explainable score, strategic balance, improve concept",
  category: "creation",
  defaultSize: "third",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "tall",
  minHeight: "short",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(18),
  dependency: ["none"],
  status: "ready",
 },
 {
  id: "community-post-studio",
  title: "Community Post Studio",
  subtitle: "Draft, generate, schedule, and publish community posts",
  category: "creation",
  defaultSize: "half",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "xtall",
  minHeight: "medium",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(19),
  dependency: ["youtube_data_v3", "gemini_api"],
  status: "ready",
 },
 {
  id: "brain-control-center",
  title: "Brain Control Center",
  subtitle: "Neural map, memory fields, signal stream, data nexus",
  category: "system",
  defaultSize: "full",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "xtall",
  minHeight: "tall",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(20),
  dependency: ["none"],
  status: "ready",
 },
 {
  id: "shorts-generator",
  title: "Shorts Generator",
  subtitle: "Ideate, plan, generate, arrange, send to editor",
  category: "creation",
  defaultSize: "half",
  minSize: "twenty-fourth",
  maxSize: "full",
  defaultHeight: "xtall",
  minHeight: "medium",
  maxHeight: "boundless",
  ...getDashboardWidgetPaletteColors(21),
  dependency: ["gemini_api"],
  status: "ready",
 },
]

export const SUPPORTED_DASHBOARD_WIDGET_IDS = DEFAULT_DASHBOARD_ROWS
 .flat()
 .map((slot) => slot.id)

const supportedOrder = new Map<string, number>(
 SUPPORTED_DASHBOARD_WIDGET_IDS.map((widgetId, index) => [widgetId, index]),
)

const inclusiveBucketRange = <T extends string>(
 order: readonly T[],
 minimum: T,
 maximum: T,
): readonly T[] => {
 const minIndex = Math.max(0, order.indexOf(minimum))
 const maxIndex = Math.max(minIndex, order.indexOf(maximum))
 return order.slice(minIndex, maxIndex + 1)
}

export const DASHBOARD_WIDGET_REGISTRY: WidgetDefinition[] = DASHBOARD_WIDGET_BASE_REGISTRY.map(
 (widget, index) => {
  const defaultSlot = defaultDashboardSlotById.get(widget.id)
  const supportedSizes = inclusiveBucketRange<DashboardSizeBucket>(
   SIZE_BUCKET_ORDER,
   widget.minSize,
   widget.maxSize,
  )
  const supportedHeights = inclusiveBucketRange<DashboardHeightBucket>(
   HEIGHT_BUCKET_ORDER,
   widget.minHeight,
   widget.maxHeight,
  )
  return {
   ...widget,
   defaultSize: defaultSlot?.size ?? widget.defaultSize,
   defaultHeight: defaultSlot?.height ?? widget.defaultHeight,
   rendererKey: widget.id,
   releaseTier: supportedOrder.has(widget.id) ? "supported" : "preview",
   defaultVisible: supportedOrder.has(widget.id) && !INITIAL_HIDDEN_IDS.has(widget.id),
   defaultOrder: supportedOrder.get(widget.id) ?? SUPPORTED_DASHBOARD_WIDGET_IDS.length + index,
   supportedSizes,
   supportedHeights,
   supportedDimensions: supportedSizes.flatMap((size) =>
    supportedHeights.map((height) => ({ size, height })),
   ),
   responsiveMode: supportedOrder.has(widget.id) ? "container" : "legacy",
  }
 },
)

if (import.meta.env.DEV) {
 assertUniqueIds(DASHBOARD_WIDGET_REGISTRY, (widget) => widget.id, "Dashboard widget registry")
}

export const DEFAULT_DASHBOARD_WIDGET_ORDER = [...DASHBOARD_WIDGET_REGISTRY]
 .sort((left, right) => left.defaultOrder - right.defaultOrder)
 .map((widget) => widget.id)

export const DASHBOARD_WIDGET_BY_ID = Object.fromEntries(
 DASHBOARD_WIDGET_REGISTRY.map((widget) => [widget.id, widget]),
)

export const WIDGET_DESCRIPTIONS: Record<string, { short: string, detailed: string }> = {
  "app-verification-explainer": {
   short: "VIEWTUBE EXPLAINS ITS PURPOSE AND DATA USE BEFORE CHANNEL TOOLS.",
   detailed: "This module is intentionally visible at the top of the dashboard so reviewers and creators can understand why the app requests Google and YouTube access."
  },
  "kpi-cluster": {
   short: "REAL-TIME CHANNEL VITALS AND CORE GROWTH METRICS.",
   detailed: "Monitor subscribers, views, and revenue in a dense, at-a-glance cluster. Best used for high-level health checks."
  },
  "overview-data-visuals": {
   short: "FORMAT SHARE + REVENUE SOURCE BREAKDOWN AT A GLANCE.",
   detailed: "Four donuts show Long-vs-Shorts share for views, watch hours, subscribers, and revenue — derived from creator content types just like the analytics page's Format Dominance visual. Three bars beneath break down the revenue dataset by source, format, and top-earning days."
  },
  "channel-overview": {
   short: "HIGH-LEVEL SUMMARY OF YOUR CHANNEL'S 28-DAY PERFORMANCE.",
   detailed: "A holistic view of your channel's reach and engagement. Use this to identify long-term trends and seasonality."
  },
  "realtime-performance": {
   short: "MONITOR LIVE CONCURRENT VIEWERS AND TRAFFIC VELOCITY.",
   detailed: "See exactly who is watching and where they come from in real time. Perfect for monitoring the first 48 hours of a launch."
  },
  "alerts-feed": {
   short: "INSTANT NOTIFICATIONS FOR MILESTONES AND CRITICAL EVENTS.",
   detailed: "Stay updated on major subscriber jumps, revenue spikes, or policy alerts. Check daily to catch urgent channel changes."
  },
  "system-micro-stack": {
   short: "MINIATURE CONTROL CENTER FOR QUICK CHANNEL ACTIONS.",
   detailed: "A compact stack for toggling visibility or quick settings. Keep this pinned for rapid-fire channel management."
  },
  "recent-uploads": {
   short: "PERFORMANCE AUDIT OF YOUR MOST RECENT VIDEO RELEASES.",
   detailed: "Compare your latest videos side-by-side. Use this to spot which topics or thumbnails are currently resonating."
  },
  "consistency-heatmap": {
   short: "VISUALIZE YOUR POSTING FREQUENCY AND CONSISTENCY.",
   detailed: "A calendar-style heat map of your uploads. Use it to ensure you aren't leaving gaps that hurt algorithmic reach."
  },
  "task-stack": {
   short: "TODO LIST FOR PRODUCTION MILESTONES AND CREATIVE GOALS.",
   detailed: "Manage your production pipeline from ideation to publish. Best used to keep track of multiple edits simultaneously."
  },
  "community-post": {
   short: "AI-POWERED GENERATOR FOR ENGAGING COMMUNITY UPDATES.",
   detailed: "Draft polls, images, and text posts optimized for reach. Best used to prime the algorithm between major video uploads."
  },
  	"description-editor": {
		short: "STANDARDIZE AND REFINE YOUR VIDEO METADATA AT SCALE.",
		detailed: "Bulk edit or template your descriptions with SEO-ready blocks. Use snippets to keep your links and socials consistent."
	},
	"script-studio": {
		short: "AI-POWERED SCRIPT EDITOR WITH RUNTIME ESTIMATION AND VOICE MATCHING.",
		detailed: "Draft, format, and refine video scripts with live runtime estimation and voice match scoring. Use AI rewrite to adapt selected text to your channel's tone."
	},
  "tag-generator": {
   short: "AI-EXTRACTED KEYWORD CLUSTERS FOR MAXIMUM SEO REACH.",
   detailed: "Generate high-ranking tags based on your video content. Best used to discover niche keywords you might have missed."
  },
  "thumb-ai": {
   short: "AI-DRIVEN THUMBNAIL ANALYSIS AND VARIATION GENERATOR.",
   detailed: "Evaluate thumbnail CTR potential and generate variations. Use it to test different emotional hooks before you publish."
  },
  "top-performer": {
   short: "DEEP DIVE INTO THE VIDEOS DRIVING YOUR CHANNEL GROWTH.",
   detailed: "Analyze your #1 videos to find the 'secret sauce' of their success. Replicate these patterns in your future content."
  },
  "revenue-momentum": {
   short: "ANALYZE EARNING VELOCITY AND FUTURE REVENUE PROJECTIONS.",
   detailed: "Track how fast you're making money and where it peaks. Use this to plan high-budget productions around revenue spikes."
  },
  "revenue-chart": {
   short: "DETAILED DAILY BREAKDOWN OF YOUR CHANNEL'S EARNINGS.",
   detailed: "A granular chart of your RPM, CPM, and total revenue. Best used to track the impact of ad-friendly content shifts."
  },
  "keyword-engine": {
   short: "SEARCH VOLUME AND COMPETITION ANALYSIS FOR NEW TOPICS.",
   detailed: "Find what viewers are searching for before you hit record. Use it to pick high-demand, low-competition video ideas."
  },
  "traffic-sources": {
   short: "MAP WHERE YOUR VIEWERS ARE DISCOVERING YOUR CONTENT.",
   detailed: "Identify if your views come from Search, Suggested, or External. Use this to tailor your titles for specific surfaces."
  },
  "audience-retention": {
   short: "DIAGNOSE EXACTLY WHERE VIEWERS STOP WATCHING VIDEOS.",
   detailed: "View second-by-second drop-off points. Use these insights to fix pacing issues and improve your video hooks."
  },
  "shorts-vs-long": {
   short: "COMPARE PERFORMANCE ACROSS DIFFERENT VIDEO FORMATS.",
   detailed: "Analyze which format drives more subs vs more revenue. Use this to balance your content strategy for maximum growth."
  },
  "superfan-card": {
   short: "IDENTIFY AND REWARD YOUR MOST LOYAL COMMUNITY MEMBERS.",
   detailed: "Track your top 10% of commenters and subscribers. Use this to engage directly with your most valuable 'inner circle.'"
  },
  "comment-replier": {
   short: "AI-ASSISTED REPLIES TO BOOST ENGAGEMENT AND RETENTION.",
   detailed: "Craft provocative, algorithm-priming replies to comments. Use it to spark longer conversations in your comment section."
  },
  "ai-prompt-box": {
   short: "DIRECT COMMAND LINE FOR CHANNEL-WIDE AI OPERATIONS.",
   detailed: "Execute complex data queries or content tasks via text. Best used for advanced batch-processing or deep data analysis."
  },
  "ask-me": {
   short: "NATURAL LANGUAGE CHAT FOR INSTANT CHANNEL INSIGHTS.",
   detailed: "Ask anything about your data and get an immediate answer. Best used for 'Why?' questions that charts can't explain."
  },
  "daily-oracle": {
   short: "STRATEGIC AI ADVICE TAILORED TO YOUR CURRENT GROWTH.",
   detailed: "Receive a daily 'prediction' or strategic tip for your channel. Use this for a fresh perspective on your content direction."
  },
  "ai-journal": {
   short: "CREATIVE LOG FOR TRACKING EXPERIMENTS AND STRATEGIES.",
   detailed: "Document what works and what doesn't in your content journey. Best used to build a personalized 'Creator Playbook.'"
  },
  "mini-calendar": {
   short: "PRODUCTION SCHEDULE AND UPCOMING MILESTONE TRACKER.",
   detailed: "A compact view of your upcoming tasks and deadlines. Use it to plan your week and avoid production bottlenecks."
  },
  "quick-actions": {
   short: "PRIMARY NAVIGATION LAUNCHERS FOR CORE TOOLS.",
   detailed: "Quickly jump to the most common dashboard surfaces. Use this as your home base for navigating the ViewTube ecosystem."
  },
  "goals-tracker": {
   short: "TRACK PROGRESS TOWARDS YOUR CHANNEL MILESTONES.",
   detailed: "Set and monitor goals for subscribers, views, or revenue. Use it to stay motivated and hit your growth targets."
  },
  "alerts-ticker": {
   short: "REAL-TIME TICKER OF CRITICAL CHANNEL EVENTS.",
   detailed: "A scrolling feed of live alerts and performance spikes. Keep an eye on this for immediate algorithmic signals."
  },
  "publish-momentum": {
   short: "HEATMAP ANALYSIS OF OPTIMAL PUBLISH TIMES.",
   detailed: "Visualize when your audience is most active. Use this to time your uploads for maximum initial velocity."
  },
  "reach-funnel": {
   short: "VISUALIZE YOUR CTR CONVERSION AND REACH EFFICIENCY.",
   detailed: "Track how impressions turn into views and watch time. Use this to identify if your titles or thumbnails are leaking traffic."
  },
  "relative-retention-benchmark": {
   short: "BENCHMARK YOUR PERFORMANCE AGAINST PLATFORM AVERAGES.",
   detailed: "See how your video retention compares to similar channels. Use this to understand if you are beating the algorithm's expectations."
  },
  "ad-stack-intelligence": {
   short: "ADVANCED MONETIZATION AND CPM ANALYSIS.",
   detailed: "Deep dive into your ad performance and monetized playbacks. Use this to optimize your content for higher-paying demographics."
  },
  "audience-matrix": {
   short: "MULTI-DIMENSIONAL VIEW OF YOUR VIEWERSHIP.",
   detailed: "Analyze geo, device, and sharing data in a unified matrix. Use this to identify new international or mobile-first growth opportunities."
  },
  "bridge-efficiency": {
   short: "MEASURE THE IMPACT OF CROSS-VIDEO PROMOTION.",
   detailed: "Track how well your end screens and cards convert viewers. Use this to build a 'bridge' that keeps viewers on your channel."
  },
  "flight-check": {
   short: "PRE-PUBLISH CHECKLIST FOR PERFECT UPLOADS.",
   detailed: "Ensure every video has optimal SEO, links, and settings before going live. Use this as a final safety net for every release."
  },
  "data-edit": {
   short: "EDIT PUBLISHED VIDEO METADATA AND MONETIZATION SETTINGS.",
   detailed: "Select a published video, review its thumbnail, then update its title, description, tags, metadata, and ad suitability settings."
  },
  "video-uploader": {
   short: "UPLOAD AND PUBLISH A NEW VIDEO.",
   detailed: "Choose a source video and thumbnail, prepare metadata and publishing options, complete ad suitability, then publish."
  },
  "image-generator": {
   short: "GENERATE THUMBNAIL + END SCREEN IMAGES WITH AI.",
   detailed: "Create images with style controls and send outputs directly to Community Post, Comment Responder, Thumb AI, and Video Manager."
  },
  "title-rewriter": {
   short: "AI-POWERED TITLE VARIATIONS AND CTR OPTIMIZATION.",
   detailed: "Generate dozens of title alternatives based on your video's core hook. Use this to test different angles before settling on a winner."
  },
  "retention-sim": {
   short: "SIMULATE VIEWERSHIP BEHAVIOR AND PREDICT DROPOFF.",
   detailed: "Analyze potential 'danger zones' in your video's pacing. Use this during the edit to cut out boring segments before you publish."
  },
  "upload-scheduler": {
   short: "7-DAY CONTENT PLANNER AND UPLOAD QUEUE.",
   detailed: "Map out your next week of content and schedule your drops. Use this to maintain a consistent rhythm without the stress."
  },
  "hashtag-analyzer": {
   short: "DEEP ANALYSIS OF HASHTAG REACH AND COMPETITION.",
   detailed: "See which hashtags are trending and which are oversaturated. Use this to pick the perfect set of 3 tags for every video."
  },
  "burnout-monitor": {
   short: "PACING TRACKER TO ENSURE CREATIVE LONGEVITY.",
   detailed: "Monitor your output vs your recovery time. Use this to avoid 'creator burnout' by identifying when you need a break."
  },
  "collab-matchmaker": {
   short: "FIND PEERS AND GENERATE COLLAB PITCHES.",
   detailed: "Discover creators with similar audiences and get AI-drafted reach-out scripts. Use this to grow your community through partnership."
  },
  "sync-controller": {
   short: "COMPACT SYNC OPERATIONS CONSOLE FOR THE DASHBOARD.",
   detailed: "Full-featured miniature of the Analytics page sync controller. Monitor dataset freshness, trigger group or unit syncs, view live queue status, and diagnose sync failures — all without leaving the dashboard."
  },
  "brain-hub": {
   short: "YOUR AI BRAIN'S MEMORY, DIRECTIVES, AND STRATEGIC ADVICE.",
   detailed: "View the Brain's evolved understanding of your channel identity, content DNA, performance, and goals. Trigger reflection cycles to update the Brain's strategic OODA directive."
  },
  "daily-command-center": {
   short: "PRIORITY ENGINE, FOCUS TIMER, AND CREATOR BRIEF IN ONE PLACE.",
   detailed: "Ranks today's action queue by urgency and impact, seeds tasks from your live channel signals, and pairs a 25-minute focus timer with a right-side creator brief so you always know the best next move."
  },
  "opportunity-desk": {
   short: "RANKED TOPIC CANDIDATES WITH EVIDENCE AND CONFIDENCE SCORES.",
   detailed: "Four lenses — Best fit, Audience asks, Search gaps, Series gaps — surface ranked opportunities pulled from your channel data. Tap two cards to compare side-by-side, then save the winner to your idea vault."
  },
  "idea-portfolio": {
   short: "EXPLAINABLE CONCEPT SCORE, STRATEGIC CHIPS, PORTFOLIO OF WINNERS.",
   detailed: "Draft a concept, tag it, and drag four dimension sliders (audience fit, novelty, demand, ease). The score-ring is the weighted mean of those inputs — no black box. Commit strong concepts to your local portfolio and generate stronger angles on demand."
  },
  "community-post-studio": {
   short: "DRAFT, GENERATE, SCHEDULE, AND PUBLISH COMMUNITY POSTS.",
   detailed: "Full lifecycle for community posts: AI-generated variants grounded in your voice + analytics, rearrangeable image collections, poll editor, best-time scheduling from real performance windows, and a copy-and-open publish flow because YouTube has no third-party community-post API."
  },
  "brain-control-center": {
   short: "NEURAL MAP + MEMORY FIELDS + SIGNAL STREAM IN ONE PANEL.",
   detailed: "Observability surface for the Brain: force-directed map of Adaptive Intelligence, Intelligence Hub, Oracle, Nexus, Super Tools, Analytics Ledger. Live signal tail, memory-field freshness with per-field reflect buttons, and export of the full state to JSON."
  },
  "shorts-generator": {
   short: "IDEATE → PLAN → GENERATE → ARRANGE → SEND TO EDITOR.",
   detailed: "Pulls a concept from the Idea Portfolio or Script Studio, drafts a 3–5 shot plan with Veo prompts, generates the clips, and lets you reorder and trim them before handing the timeline off to the video editor. Also supports algo-priming mode for a target video launch."
  },
 };
