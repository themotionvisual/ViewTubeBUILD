...
import { BarChart3, Brain, Database, ExternalLink, MessageSquare, Package, Radar, RefreshCw, Send, Settings2, ShieldCheck, Sparkles, Target, Zap } from "lucide-react"
...
export const BrainHubWidget: React.FC<BrainHubWidgetProps> = ({ data, ...common }) => {
 ...
 const snapshot=useMemo(()=>buildAIBrainContextSnapshot({brain,authState,channelConnection,brainMemory:controls.personalization?getBrainMemory():null,recentConversationTurns:controls.personalization?turns:[]}),[brain,authState,channelConnection,controls.personalization,turns])
 ...
 const evidence=(snapshot.evidencePack?.items||[]).slice(0,engines.maxEvidenceItems)
 ...
 const tabs:[Tab,string,any][]=[["chat","Chat",MessageSquare],["intelligence","Intel",Target],["evidence","Evidence",Database],["packages","Packages",Package]]
 return <WidgetShell {...common} icon={<Brain size={22}/>}>
  <div style={{display:"flex",flexDirection:"column",height:"100%",minHeight:0,gap:7,color:"color-mix(in srgb, var(--widget-color) 52%, #3b1020)"}}>
   <div style={{display:"grid",gridTemplateColumns:"repeat(4,minmax(0,1fr))",gap:4}}>{tabs.map(([id,label,Icon])=><button key={id} onClick={()=>setTab(id)} style={pill(tab===id,"color-mix(in srgb, var(--widget-color) 42%, white)")}><Icon size={10} style={{display:"inline",marginRight:3}}/>{label}</button>)}</div>
   ...
   <div style={{display:"grid",gridTemplateColumns:"1fr 34px",gap:5}}><input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")void send()}} disabled={!controls.enabled} placeholder={controls.enabled?"Ask Brain…":"Brain disabled"} style={{height:32,border:"2px solid currentColor",borderRadius:7,padding:"0 8px",fontSize:10,fontWeight:800,outline:"none",minWidth:0}}/><button onClick={()=>void send()} disabled={!input.trim()||busy||!controls.enabled} style={{height:32,border:"2px solid currentColor",borderRadius:7,background:"#b9f536",display:"grid",placeItems:"center",opacity:busy?.55:1}} aria-label="Send"><Send size={13}/></button></div>
   <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:5}}><span style={{fontSize:7,fontWeight:1000,textTransform:"uppercase",opacity:.5}}><ShieldCheck size={9} style={{display:"inline"}}/> {controls.externalActionsRequireApproval?"Approval gated":"Approval policy relaxed"}</span><Link to="/brain-controls" style={{...pill(true,"color-mix(in srgb,var(--widget-color) 24%,white)") as any,display:"inline-flex",alignItems:"center",gap:3,textDecoration:"none"}}><Settings2 size={9}/> Controls <ExternalLink size={8}/></Link></div>
  </div>
 </WidgetShell>
}
