...
const ThumbnailStudio: React.FC<ThumbnailStudioProps> = ({ embedded = false }) => {
    ...
    return (
        <div className={`${embedded ? 'w-full' : 'min-h-screen w-full bg-[#f8fafc] flex flex-col p-8 overflow-y-auto custom-scrollbar animate-fade-in'}`}>

            {/* Main Toolbox Container (White Interior) */}
            <div className={`${embedded ? 'w-full bg-white' : 'w-full max-w-[1600px] mx-auto bg-white border-[6px] border-black rounded-[24px] shadow-[8px_8px_0px_0px_black] flex flex-col overflow-visible'}`}>
                ...
                {/* Toolbox Content */}
                <main className={`flex-1 bg-white ${embedded ? 'p-0' : 'p-12'}`}>
