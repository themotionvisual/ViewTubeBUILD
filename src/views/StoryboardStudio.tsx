...
import { 
  Plus, Trash2, Image as ImageIcon, Sparkles, AlertTriangle, 
  Activity, Video, Type, Settings2, Eye, Cloud
} from 'lucide-react';
...
const StoryboardStudio: React.FC = () => {
  const { brain, registerProvider, unregisterProvider, setStoryboardState } = useBrain();
  ...
  const [scenes, setScenes] = useState<Scene[]>([
    {
      id: '1',
      name: 'The Hook',
      text: 'Have you ever wondered why the French cavalry was completely unstoppable under Napoleon? Today, we reveal the tactical secret.',
      broll: 'Drone shot charging across a field, transitioning to a vintage painting of cuirassiers.',
      imageUrl: null,
      emotionScore: calculateEmotion('Have you ever wondered why the French cavalry was completely unstoppable under Napoleon?!'),
      durationEstimate: calculateDuration('Have you ever wondered why the French cavalry was completely unstoppable under Napoleon? Today, we reveal the tactical secret.')
    },
    {
      id: '2',
      name: 'The Context',
      text: 'Before 1805, European cavalry units were fragmented and misused. They were lightly armored and thrown into battles without a cohesive plan.',
      broll: 'Slow pan across a map of Europe with markers fading out.',
      imageUrl: null,
      emotionScore: calculateEmotion('Before 1805, European cavalry units were fragmented.'),
      durationEstimate: calculateDuration('Before 1805, European cavalry units were fragmented and misused. They were lightly armored and thrown into battles without a cohesive plan.')
    }
  ]);
  ...
  const warnings = useMemo(() => {
    ...
    scenes.forEach((s, idx) => {
      if (s.durationEstimate > 20 && s.broll.length < 10) {
        w.push(`${s.name || 'Scene ' + (idx + 1)} is ${s.durationEstimate}s long with almost no visual changes planned. Add B-roll!`);
      }
      if (s.durationEstimate < 2) {
        w.push(`${s.name || 'Scene ' + (idx + 1)} is too short (under 2s). Hard to pace visually.`);
      }
    });
    return w;
  }, [scenes]);
  ...
  useEffect(() => {
    setStoryboardState({
      ...
      pacingHealth: warnings.length > 2 ? 'Critical' : warnings.length > 0 ? 'Warning' : 'Excellent'
    });
  }, [scenes, totalDuration, warnings.length, setStoryboardState]);
  ...
  const handleEnhance = async (sceneId: string, audioUrl: string, title: string) => {
    setIsEnhancingId(sceneId);
    try {
      const response = await auphonicService.startProduction(audioUrl, title);
      ...
      while (status !== 'done' && status !== 'error' && attempts < 10) {
         const poll = await auphonicService.getProductionStatus(uuid);
         status = poll.data?.status_string || 'error';
         if (status === 'done') {
            updateScene(sceneId, 'voiceoverUrl', poll.data.output_files?.[0]?.download_url);
         }
  ...
  const chartData = useMemo(() => scenes.map((s, i) => ({
    name: s.name || `S${i+1}`,
      ...
      {brain.seoState.winningTitle && (
        <div className="mb-4 bg-white border-[3px] border-black rounded-lg p-3 shadow-[4px_4px_0px_0px_black] flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span className="bg-[#00d2ff] text-black px-2 py-1 text-xs font-black uppercase rounded border-2 border-black">Target Topic</span>
            <span className="font-bold italic text-xl">"{brain.seoState.winningTitle}"</span>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-row flex-1 gap-4 overflow-hidden">
        ...
        <div className="w-[30%] bg-white border-[3px] border-black rounded-xl shadow-[6px_6px_0px_0px_black] flex flex-col overflow-hidden">
          <div className="bg-black text-white p-3 flex justify-between items-center z-10">
            <h2 className="font-bold flex items-center gap-2"><Type size={16}/> THE BLUEPRINT</h2>
             <div className="flex items-center gap-4">
                ...
                >
                  {isSyncing ? <div className="w-3 h-3 border-2 border-black border-t-transparent rounded-full animate-spin" /> : <Cloud size={14} />}
                  Sync
                </button>
                <span className="text-[#00ff99] text-sm font-mono font-bold">Total: {formatTime(totalDuration)}</span>
               <button onClick={addScene} className="bg-[#ccff00] text-black px-2 py-1 text-xs font-bold rounded border-2 border-transparent hover:border-black transition-all"><Plus size={14}/></button>
            </div>
          </div>
          <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50">
            {scenes.map((scene) => (
              <div key={scene.id} className="border-[3px] border-black p-3 bg-white rounded-lg relative group shadow-sm hover:shadow-[4px_4px_0px_0px_#e5e7eb] transition-all">
                <input 
                  type="text"
                ...
                <div className="mt-2 flex justify-between items-center text-xs font-bold text-gray-500 bg-gray-100 p-1.5 rounded">
                  <span className="font-mono text-black">{formatTime(scene.durationEstimate)}</span>
                  <button onClick={() => deleteScene(scene.id)} className="text-red-500 opacity-0 group-hover:opacity-100 transition-opacity hover:scale-110">
        ...
        <div className="flex-1 bg-white border-[3px] border-black rounded-xl shadow-[6px_6px_0px_0px_black] flex flex-col overflow-hidden">
          <div className="bg-black text-[#ccff00] p-3 border-b-[3px] border-black flex items-center justify-between z-10">
            <div className="flex items-center gap-2">
              <Eye size={16}/> <h2 className="font-bold uppercase tracking-tight">Visual Canvas</h2>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 bg-[#f3f4f6]">
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
              {scenes.map((scene) => (
                <div key={scene.id} className="bg-white border-[3px] border-black rounded-xl shadow-[4px_4px_0px_0px_rgba(0,0,0,0.1)] flex flex-col overflow-hidden transform hover:-translate-y-1 transition-transform">
                  <div className="bg-gradient-to-r from-black to-gray-800 text-white px-3 py-1.5 flex justify-between items-center border-b-[3px] border-black">
                    ...
                    <span className="text-[#00d2ff] text-xs font-mono font-bold">{formatTime(scene.durationEstimate)}</span>
                  </div>
                  
                  {/* Image Slot */}
                  <div className="aspect-video bg-gray-100 flex items-center justify-center border-b-[3px] border-black relative group cursor-pointer overflow-hidden">
                    {scene.imageUrl ? (
                      <img src={scene.imageUrl} alt="Scene Visual" className="w-full h-full object-cover" />
                    ) : (
                      <div className="flex flex-col items-center text-gray-400 group-hover:text-black transition-colors">
                        <ImageIcon size={32} />
                        <span className="text-xs font-bold mt-2 uppercase tracking-wider">Visual Concept</span>
                      </div>
                    )}
                    <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                      <button className="bg-[#ccff00] text-black px-4 py-2 rounded font-black border-[3px] border-black flex items-center gap-2 transform hover:scale-105 active:scale-95 transition-all">
                  ...
                  <div className="p-3 bg-black/5 border-t-[3px] border-black flex items-center justify-between gap-4">
                     {scene.voiceoverUrl ? (
                         <div className="flex-1 flex flex-col gap-2">
                            <div className="flex items-center gap-3">
                               <audio src={scene.voiceoverUrl} hide-chrome-interface-labels="true" controls className="h-8 flex-1 scale-90 origin-left" />
                               <div className="bg-[#ccff00] text-black p-2 rounded-lg border-2 border-black">
                                  <CustomIcon name="headset" size={14} />
                               </div>
                            </div>
                            <button 
                              onClick={() => handleEnhance(scene.id, scene.voiceoverUrl!, scene.name)}
                        ...
                        <>
                           <div className="flex items-center gap-2 text-[10px] font-black tracking-widest text-black/50 uppercase">
                              <CustomIcon name="mic" size={12} className="opacity-50" /> Voiceover Nexus
                           </div>
                           <button 
                             onClick={() => generateVoiceover(scene.id, scene.text)}
                             disabled={generatingVoiceId === scene.id || !scene.text}
                             className="bg-black text-[#ccff00] px-3 py-1.5 rounded-lg text-[10px] font-black uppercase italic tracking-widest flex items-center gap-2 hover:scale-105 active:scale-95 transition-all disabled:opacity-50"
                           >
        ...
        <div className="w-[25%] bg-white border-[3px] border-black rounded-xl shadow-[6px_6px_0px_0px_black] flex flex-col overflow-hidden">
          <div className="bg-[#00d2ff] text-black border-b-[3px] border-black p-3 flex items-center justify-between z-10">
            <div className="flex items-center gap-2">
              <Settings2 size={16}/><h2 className="font-black uppercase tracking-tight">The Oracle</h2>
            </div>
            {warnings.length === 0 ? (
               <div className="bg-black text-[#00ff99] px-2 py-0.5 rounded textxs font-bold font-mono text-[10px]">ALL CLEAR</div>
            ) : (
               <div className="bg-black text-[#ff3399] px-2 py-0.5 rounded textxs font-bold font-mono text-[10px] animate-pulse">{warnings.length} WARNINGS</div>
            )}
          </div>
          ...
          <div className="flex-1 overflow-y-auto flex flex-col bg-gray-50">
            ...
            <div className="p-4 border-b-[3px] border-black bg-white">
              <h3 className="font-black italic mb-3 flex items-center gap-2 uppercase text-sm"><Activity size={16} className="text-[#ff3399]" /> Emotion Arc</h3>
              <div className="h-40 w-full bg-black rounded-xl p-3 relative border-[3px] border-black shadow-[4px_4px_0px_0px_#ccff00]">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={chartData}>
                    ...
                    <Tooltip 
                      contentStyle={{ backgroundColor: '#000', color: '#fff', border: '3px solid #00d2ff', borderRadius: '8px', fontWeight: 'bold' }}
                      itemStyle={{ color: '#00ff99', fontWeight: 'black' }}
                    />
                    <Area type="monotone" dataKey="passion" stroke="#00d2ff" strokeWidth={4} fillOpacity={1} fill="url(#colorPassion)" />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
              <p className="text-[10px] text-gray-500 mt-3 font-bold uppercase tracking-tight leading-tight">Visualizes script intensity to prevent "flatlining". Values above 50 indicate high excitement/pacing.</p>
            </div>

            {/* Retention Warnings */}
            <div className="p-4 flex-1">
              <h3 className="font-black italic text-black mb-3 flex items-center gap-2 text-sm uppercase"><AlertTriangle size={16} className={warnings.length > 0 ? "text-red-500" : "text-black"}/> Retentions Flags</h3>
              {warnings.length === 0 ? (
                ...
                <div className="space-y-3">
                  {warnings.map((w, i) => (
                    <div key={i} className="bg-white border-[3px] border-black p-3 rounded-lg shadow-[3px_3px_0px_0px_#ff3399] text-xs font-bold flex items-start gap-2">
                       <span className="shrink-0 text-[#ff3399] leading-none mt-0.5">●</span>
