...
const Channelytics: React.FC = () => {
  ...
  const setUnifiedCsvFiles = (nextFiles: CsvFileWithTag[]) => {
    updateBrain({
      channelyticsState: { ...brain.channelyticsState, csvFiles: nextFiles },
    });
  ...
  const processFiles = async (files: CsvFileWithTag[], initialDateRange?: string) => {
    ...
    if (dataSource === 'csv' || dataSource === 'both') {
      for (const f of files) {
        const parsed = (f.data || []).map((row: any, idx: number) => {
          ...
          const enriched = normalizeAndEnrichRow(newRow);
          return {
            ...newRow,
            ...enriched,
    ...
    if (dataSource === 'analytics' || dataSource === 'both') {
      try {
        const canonicalRows = getMasterRows('lifetime', 'api');
        const apiRows = canonicalRowsToMasterTableRows(canonicalRows).map((row, idx) =>
          normalizeAndEnrichRow({
            ...row,
            _sourceFile: row._sourceFile || 'YouTube API',
            _userTag: row._userTag || 'unknown',
            _id: row._id || `yt-api-${idx}`,
          }),
        );
        combined = [...combined, ...apiRows];
      } catch (e) { console.error("Failed to parse yt_analytics_cache", e); }
    }

    const deduped = dedupeByVideoDate(combined);
    setParsedData(prev => JSON.stringify(prev) === JSON.stringify(deduped) ? prev : deduped);
  };
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setIsUploading(true);
    try {
      const { csvFiles, extractedDateRange } = await expandCsvAndZipFiles(Array.from(files));
      if (csvFiles.length === 0) return;

      const newFilesWithTags = await buildCsvFilesWithTags(csvFiles, preUploadType);
      ...
      if (extractedDateRange) {
        ...
        await processFiles(updatedFiles);
      }
    } finally {
  ...
  const clearAll = () => {
    ...
    setResearchLabState({ csvFiles: [], analyticsResult: null });
  };
          ...
          <div className="flex flex-col lg:flex-row items-start lg:items-center gap-6 mb-8 px-1">
            ...
            <div className="flex flex-wrap items-center gap-4 w-full lg:w-auto">
              <div className="relative flex-1 lg:flex-none">
                <select
                  value={preUploadType}
                  onChange={(e) => setPreUploadType(e.target.value as CsvUploadType)}
