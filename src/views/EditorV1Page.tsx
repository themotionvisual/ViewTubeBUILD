...
const EditorUiMenu: React.FC<{legacyMode:EditorFrontendMode;onLegacyModeChange:(v:EditorFrontendMode)=>void;frontend:EditorFrontend;onFrontendChange:(v:EditorFrontend)=>void;layout:EditorLayoutChoice;onLayoutChange:(v:EditorLayoutChoice)=>void;aspect:CompositionAspect;onAspectChange:(v:CompositionAspect)=>void;}> = ({legacyMode,onLegacyModeChange,frontend,onFrontendChange,layout,onLayoutChange,aspect,onAspectChange}) => {
  ...
  return <div className="pointer-events-auto absolute right-2 top-2 z-[140] flex flex-col items-end gap-1"><button type="button" aria-haspopup="menu" aria-expanded={open} onClick={()=>setOpen(v=>!v)} className="flex h-8 items-center gap-2 rounded-[7px] border-[2px] border-black bg-white px-2.5 text-[9px] font-black uppercase tracking-[0.08em] text-black"><span>⚙</span><span>Editor UI</span><span className="rounded-[4px] border border-black bg-[#40C6E9] px-1.5 py-0.5 text-[7px]">{summary}</span></button>{open&&<div role="menu" aria-label="Editor UI settings" className="w-[292px] max-w-[calc(100vw-16px)] rounded-[8px] border-[3px] border-black bg-[#f0f0f4] p-2 text-black shadow-[3px_3px_0_rgba(0,0,0,.28)]"><div className="mb-2 flex items-center justify-between border-b-2 border-black pb-1.5"><div><div className="text-[10px] font-black uppercase tracking-[0.12em]">Editor UI</div><div className="text-[8px] font-bold text-black/55">Change the view without changing the edit.</div></div><button type="button" onClick={()=>setOpen(false)} className="h-6 w-6 rounded-[4px] border-2 border-black bg-white text-[12px] font-black">×</button></div><div className="grid gap-2"><MiniGroup label="Interface" value={frontend} onChange={onFrontendChange} options={[{value:'auto',label:'Auto'},{value:'mobile',label:'Mobile'},{value:'desktop',label:'Desktop'}]}/><MiniGroup label="Layout" value={layout} onChange={onLayoutChange} options={[{value:'auto',label:'Auto'},{value:'portrait',label:'Upright'},{value:'landscape',label:'Sideways'}]}/><MiniGroup label="Video" value={aspect} onChange={onAspectChange} options={[{value:'portrait',label:'9:16'},{value:'landscape',label:'16:9'}]}/><div className="border-t-2 border-black/15 pt-2"><div className="mb-1 text-[7px] font-black uppercase tracking-[0.12em] text-black/55">Editor Style</div><div className="grid grid-cols-2 gap-1">{EDITOR_FRONTEND_MODES.map(item=><button key={item.id} type="button" onClick={()=>onLegacyModeChange(item.id)} aria-pressed={item.id===legacyMode} className="min-h-8 rounded-[5px] border-2 border-black px-2 text-left text-[8px] font-black uppercase" style={{background:item.id===legacyMode?'#FFFF61':'#fff'}}>{item.shortLabel||item.label}</button>)}</div><div className="mt-1 text-[7px] font-bold text-black/45">{activeLegacy.label} · saved on this device</div></div></div></div>}</div>;
};
