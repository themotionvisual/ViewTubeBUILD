...
const CrownControlRoom: React.FC = () => {
  ...
  return (
    <main className="min-h-full bg-[#efefe8] px-3 py-4 text-black sm:px-5 sm:py-6 lg:px-7">
      ...
      {section === "tasks" ? <section className="mt-4 grid gap-4 lg:grid-cols-[.75fr_1.25fr]"><ReaderHeader title="Tasks" note="Task status remains owned by the canonical external Task Index. This reader exposes the authority boundary instead of fabricating browser-side task state." tone="#ffda47" /><div className={`${cardClass} p-5`}><div className="text-[10px] font-black uppercase tracking-[.14em]">Canonical authority</div><div className="mt-2 break-all text-sm font-black">{CROWN_TASK_AUTHORITY.authority}</div><p className="mt-3 text-xs font-bold leading-5">{CROWN_TASK_AUTHORITY.note}</p><div className="mt-5 grid gap-3 sm:grid-cols-2">{CROWN_MISSIONS.map((mission) => <div key={mission.id} className="rounded-xl border-[2px] border-black p-3"><div className="text-[10px] font-black uppercase">{mission.domain}</div><div className="mt-1 text-xs font-black">{mission.id}</div><div className="mt-2 text-[11px] font-bold text-black/60">Task refs: {mission.taskIds.length ? mission.taskIds.join(", ") : "none recorded"}</div></div>)}</div></div></section> : null}

      {section === "artifacts" ? <section className="mt-4 grid gap-4 lg:grid-cols-[.75fr_1.25fr]"><ReaderHeader title="Artifacts" note="Artifact cards are generated from committed Crown artifact records and explicit mission references. Provenance remains visible." tone="#36e0f6" /><div className="grid gap-3">{CROWN_ARTIFACTS.map((artifact) => <article key={artifact.id} className={`${cardClass} p-4`}><div className="flex flex-wrap items-center justify-between gap-2"><h3 className="text-sm font-black uppercase">{artifact.title}</h3><span className="rounded-full border-[2px] border-black bg-[#d7fff2] px-2 py-1 text-[9px] font-black uppercase">{artifact.status}</span></div><div className="mt-2 break-all text-xs font-bold">{artifact.source}</div><div className="mt-2 text-[10px] font-black uppercase text-black/50">{artifact.id} · {artifact.verified ? "verified reference" : "not runtime verified"}</div></article>)}</div></section> : null}

      {section === "decisions" ? <section className="mt-4 grid gap-4 lg:grid-cols-[.75fr_1.25fr]"><ReaderHeader title="Decisions" note="Decision records preserve the reason a path was chosen, not just the chosen result." tone="#f1dcff" /><div className="grid gap-3">{CROWN_DECISIONS.map((decision) => <article key={decision.id} className={`${cardClass} p-5`}><div className="flex flex-wrap justify-between gap-2"><h3 className="max-w-3xl text-lg font-black uppercase leading-tight">{decision.question}</h3><span className="rounded-full border-[2px] border-black bg-[#c0f240] px-2 py-1 text-[9px] font-black uppercase">{decision.state}</span></div><p className="mt-4 text-sm font-bold leading-5">{decision.recommendation}</p><div className="mt-4 text-[10px] font-black uppercase text-black/50">{decision.id} · {decision.evidenceCount} evidence statements · {decision.missionId}</div></article>)}</div></section> : null}

      {section === "execution" ? <section className="mt-4 grid gap-4 lg:grid-cols-[.75fr_1.25fr]"><ReaderHeader title="Execution" note="Each mission has an EMPEROR work-order record. Presence proves an execution packet exists; it does not prove runtime work completed." tone="#ffa85c" /><div className="grid gap-3">{CROWN_EXECUTION.map((item) => <article key={item.record} className={`${cardClass} p-4`}><div className="text-[10px] font-black uppercase tracking-[.14em]">Work order recorded</div><div className="mt-2 text-sm font-black">{item.missionId}</div><div className="mt-2 break-all text-[11px] font-bold text-black/55">{item.record}</div></article>)}</div></section> : null}
